...
export async function seedLifeLoggerData() {
  console.log('[LifeLogger Seed] 开始生成示例数据...');
  ...
  for (const record of healthRecords) {
    await db.insert(healthMetrics).values(record);
  }
  console.log(`[LifeLogger Seed] 已插入 ${healthRecords.length} 条健康数据`);
  ...
  for (const record of socialRecords) {
    await db.insert(socialInteractions).values(record as any);
  }
  console.log(`[LifeLogger Seed] 已插入 ${socialRecords.length} 条社交互动记录`);
  ...
  for (const record of patternRecords) {
    await db.insert(behaviorPatterns).values(record as any);
  }
  console.log(`[LifeLogger Seed] 已插入 ${patternRecords.length} 条行为模式记录`);
  
  console.log('[LifeLogger Seed] 示例数据生成完成！');
