...
function broadcastCommandStatus(
  connectedUsers: Map<WebSocket, any>,
  command: any
) {
  const message = {
    type: 'COMMAND_STATUS',
...
export const registerZ3DevicesRoutes: RegisterRouteFn = (
  ...
  app.post("/api/z3/devices/:id/heartbeat", async (req, res) => {
    try {
      const device = await storage.updateDeviceHeartbeat(req.params.id);
      if (!device) {
        return res.status(404).json({ error: "Device not found" });
      }

      const pendingCommands = await storage.getPendingCommands(req.params.id);
      res.json({
        device,
        pendingCommands,
      });
  ...
  app.post("/api/z3/devices/:id/commands", requireMaster, async (req, res) => {
    try {
      ...
      const command = await storage.createRemoteCommand({
  ...
  app.get("/api/z3/devices/:id/commands", requireMaster, async (req, res) => {
    try {
      ...
      const commands = await storage.getDeviceCommands(req.params.id, limit);
      res.json(commands);
    } catch (error) {
  ...
  app.post("/api/z3/commands/:id/ack", async (req, res) => {
    try {
      ...
      const updates: Record<string, any> = { status: status || 'ACKNOWLEDGED' };
      ...
      const command = await storage.updateRemoteCommand(req.params.id, updates);
      if (!command) {
  ...
  app.post("/api/z3/devices/:id/request-primary", requireMaster, async (req, res) => {
    try {
      const requestingDevice = await storage.getDevice(req.params.id);
      if (!requestingDevice) {
        return res.status(404).json({ error: "Device not found" });
      }

      const allDevices = await storage.getAllDevices(requestingDevice.userId);
      ...
      if (currentPrimary && currentPrimary.id !== req.params.id) {
        await storage.updateDevice(currentPrimary.id, { status: 'ONLINE' });
      ...
      await storage.updateDevice(req.params.id, { status: 'BUSY' });
  ...
  app.post("/api/z3/devices/resolve-conflict", requireMaster, async (req, res) => {
    try {
      ...
      const validDevices = devices.filter(Boolean) as any[];
      ...
      for (const device of validDevices) {
        try {
          if (device.id === winner.id) {
            await storage.updateDevice(device.id, { status: 'BUSY' });
          } else {
  ...
  app.post("/api/z3/presence/voice-event", requireMaster, async (req, res) => {
    try {
      ...
      const masterVoice = await storage.getMasterVoiceprint?.();
