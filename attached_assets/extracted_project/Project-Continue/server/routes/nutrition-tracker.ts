...
router.post('/meal', async (req: Request, res: Response) => {
  try {
    ...
    console.error('[NutritionTracker API] 记录餐食失败:', error);
...
router.post('/water', async (req: Request, res: Response) => {
  try {
    ...
    console.error('[NutritionTracker API] 记录饮水失败:', error);
    if (error instanceof z.ZodError) {
...
router.post('/caffeine', async (req: Request, res: Response) => {
  try {
    ...
    console.error('[NutritionTracker API] 记录咖啡因失败:', error);
...
router.get('/summary', async (req: Request, res: Response) => {
  try {
    ...
    console.error('[NutritionTracker API] 获取每日摘要失败:', error);
...
router.get('/goals', async (req: Request, res: Response) => {
  try {
    ...
    console.error('[NutritionTracker API] 获取目标失败:', error);
...
router.put('/goals', async (req: Request, res: Response) => {
  try {
    ...
    console.error('[NutritionTracker API] 更新目标失败:', error);
...
router.get('/history', async (req: Request, res: Response) => {
  try {
    ...
    console.error('[NutritionTracker API] 获取历史记录失败:', error);
...
router.get('/trend', async (req: Request, res: Response) => {
  try {
    ...
    console.error('[NutritionTracker API] 获取趋势失败:', error);
...
router.post('/suggest', async (req: Request, res: Response) => {
  try {
    ...
    console.error('[NutritionTracker API] 生成建议失败:', error);
    if (error instanceof z.ZodError) {
      res.status(400).json({ success: false, error: '参数格式错误' });
    } else {
