...
export function registerHybridAIRoutes(app: Express, storage: IStorage): void {
  ...
  app.get("/api/model/status", async (req: Request, res: Response) => {
    try {
      ...
      console.error('[HybridAI] Model status error:', error);
  ...
  app.post("/api/devices/register", requireMaster, async (req: Request, res: Response) => {
    try {
      ...
      const userId = (req as any).userId || 'master';
      const userRole = (req as any).userRole || 'MASTER';
      ...
      console.error('[HybridAI] Device register error:', error);
  ...
  app.post("/api/queue/jobs", requireMaster, async (req: Request, res: Response) => {
    try {
      ...
      console.error('[HybridAI] Job add error:', error);
  ...
  app.post("/api/ollama/pull", requireMaster, async (req: Request, res: Response) => {
    try {
      ...
      ollamaAdapter.pullModel(model, (progress) => {
        console.log(`[Ollama] Pull progress: ${(progress * 100).toFixed(1)}%`);
      }).then(success => {
        console.log(`[Ollama] Pull ${model}: ${success ? 'SUCCESS' : 'FAILED'}`);
      }).catch(err => {
  ...
  app.post("/api/servers", requireMaster, async (req: Request, res: Response) => {
    try {
      ...
      console.error('[HybridAI] Add server error:', error);
  ...
  app.post("/api/servers/test", requireMaster, async (req: Request, res: Response) => {
    try {
      ...
      console.error('[HybridAI] Server test error:', error);
  ...
  console.log('[HybridAI] Routes registered (with GPU server support)');
}
