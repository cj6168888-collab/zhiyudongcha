...
router.get('/soul/state/:masterId', async (req, res) => {
  try {
    ...
    console.error('[PersonalityCore] Get soul state error:', error);
    res.status(500).json({
      success: false,
...
router.post('/soul/initialize', requireMaster, async (req, res) => {
  try {
    ...
    console.error('[PersonalityCore] Initialize master error:', error);
    res.status(500).json({
      success: false,
...
router.post('/soul/transfer', requireMaster, async (req, res) => {
  try {
    ...
    console.error('[PersonalityCore] Transfer loyalty error:', error);
    res.status(500).json({
      success: false,
...
router.post('/soul/delegate', async (req, res) => {
  try {
    ...
    console.error('[PersonalityCore] Delegate error:', error);
    res.status(500).json({
      success: false,
...
router.get('/soul/nostalgia/:masterId', async (req, res) => {
  try {
    ...
    console.error('[PersonalityCore] Check nostalgia error:', error);
    res.status(500).json({
      success: false,
...
router.get('/state', async (_req, res) => {
  try {
    ...
    console.error('[PersonalityCore] Get state error:', error);
    res.status(500).json({
      success: false,
...
router.post('/state/persona', async (req, res) => {
  try {
    ...
    console.error('[PersonalityCore] Switch persona error:', error);
    res.status(500).json({
      success: false,
...
router.post('/state/medium', async (req, res) => {
  try {
    ...
    console.error('[PersonalityCore] Switch medium error:', error);
    res.status(500).json({
      success: false,
      message: error instanceof Error ? error.message : '切换失败',
    });
...
router.post('/state/emotion', async (req, res) => {
  try {
    ...
    console.error('[PersonalityCore] Update emotion error:', error);
    res.status(500).json({
      success: false,
...
router.post('/capability/assess', async (req, res) => {
  try {
    ...
    console.error('[PersonalityCore] Assess capability error:', error);
    res.status(500).json({
      success: false,
...
router.post('/promise/create', async (req, res) => {
  try {
    ...
    console.error('[PersonalityCore] Create promise error:', error);
    res.status(500).json({
      success: false,
...
router.post('/promise/:promiseId/progress', async (req, res) => {
  try {
    ...
    console.error('[PersonalityCore] Update progress error:', error);
    res.status(500).json({
      success: false,
      message: error instanceof Error ? error.message : '更新失败',
    });
...
router.get('/promise/active', async (_req, res) => {
  try {
    ...
    console.error('[PersonalityCore] Get active promises error:', error);
    res.status(500).json({
      success: false,
...
router.get('/promise/risks', async (_req, res) => {
  try {
    ...
    console.error('[PersonalityCore] Check risks error:', error);
    res.status(500).json({
      success: false,
      message: error instanceof Error ? error.message : '检查失败',
    });
...
router.get('/prompt/:masterId', async (req, res) => {
  try {
    ...
    console.error('[PersonalityCore] Get prompt error:', error);
    res.status(500).json({
      success: false,
...
router.get('/prompt/:masterId/structured', async (req, res) => {
  try {
    ...
    console.error('[PersonalityCore] Get structured prompt error:', error);
    res.status(500).json({
      success: false,
      message: error instanceof Error ? error.message : '生成失败',
    });
...
router.post('/social/register', async (req, res) => {
  try {
    ...
    console.error('[PersonalityCore] Register identity error:', error);
    res.status(500).json({
      success: false,
...
router.post('/social/:identityId/confirm', async (req, res) => {
  try {
    ...
    console.error('[PersonalityCore] Confirm identity error:', error);
    res.status(500).json({
      success: false,
...
router.post('/social/:identityId/encounter', async (req, res) => {
  try {
    ...
    console.error('[PersonalityCore] Record encounter error:', error);
    res.status(500).json({
      success: false,
...
router.get('/social/identities', async (_req, res) => {
  try {
    ...
    console.error('[PersonalityCore] Get identities error:', error);
    res.status(500).json({
      success: false,
      message: error instanceof Error ? error.message : '获取失败',
    });
...
router.post('/social/search', async (req, res) => {
  try {
    ...
    console.error('[PersonalityCore] Search identity error:', error);
    res.status(500).json({
      success: false,
...
router.post('/social/analyze-audio', async (req, res) => {
  try {
    ...
    console.error('[PersonalityCore] Analyze audio error:', error);
    res.status(500).json({
      success: false,
...
router.post('/social/propose-registration', async (req, res) => {
  try {
    ...
    console.error('[PersonalityCore] Propose registration error:', error);
    res.status(500).json({
      success: false,
...
router.post('/social/restore-private', async (_req, res) => {
  try {
    ...
    console.error('[PersonalityCore] Restore private error:', error);
    res.status(500).json({
      success: false,
...
router.post('/social/extract-names', async (req, res) => {
  try {
    ...
    console.error('[PersonalityCore] Extract names error:', error);
    res.status(500).json({
      success: false,
