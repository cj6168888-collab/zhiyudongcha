...
router.get('/logs', async (req: Request, res: Response) => {
  try {
    ...
    console.error('[DreamLogs] 获取梦境日志失败:', error);
    res.status(500).json({ 
      success: false, 
...
router.get('/logs/latest', async (req: Request, res: Response) => {
  try {
    ...
    console.error('[DreamLogs] 获取最新梦境日志失败:', error);
    res.status(500).json({ 
      success: false, 
...
router.get('/logs/:id', async (req: Request, res: Response) => {
  try {
    ...
    console.error('[DreamLogs] 获取梦境日志详情失败:', error);
    res.status(500).json({ 
      success: false, 
...
router.get('/stats', async (req: Request, res: Response) => {
  try {
    ...
    console.error('[DreamLogs] 获取梦境统计失败:', error);
    res.status(500).json({ 
      success: false, 
...
router.get('/insights', async (req: Request, res: Response) => {
  try {
    ...
    console.error('[DreamLogs] 获取对话洞察失败:', error);
    res.status(500).json({ 
      success: false, 
...
router.post('/run', async (req: Request, res: Response) => {
  try {
    ...
    console.error('[DreamLogs] 手动触发梦境整理失败:', error);
    res.status(500).json({ 
      success: false, 
...
router.get('/status', async (req: Request, res: Response) => {
  try {
    ...
    console.error('[DreamLogs] 获取梦境状态失败:', error);
    res.status(500).json({ 
      success: false, 
