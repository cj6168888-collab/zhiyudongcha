...
async function simulateIntegrationSync(jobId: string, storage: IStorage) {
  const stages = [
    { status: "running", itemsProcessed: 0 },
    { status: "running", itemsProcessed: 25 },
    { status: "running", itemsProcessed: 50 },
    { status: "running", itemsProcessed: 75 },
    { status: "completed", itemsProcessed: 100 },
  ];
  
  await storage.updateIntegrationSyncJob(jobId, { startedAt: new Date() });
  
  for (const stage of stages) {
    await new Promise(resolve => setTimeout(resolve, 1000));
    await storage.updateIntegrationSyncJob(jobId, {
      ...stage,
...
export function registerIntegrationsRoutes(app: Express, storage: IStorage) {
  ...
  app.get("/api/integrations/providers", async (req, res) => {
    try {
      ...
      const providers = await storage.getAllIntegrationProviders(category);
      res.json(providers);
    } catch (error) {
  ...
  app.get("/api/integrations/accounts", async (req, res) => {
    try {
      ...
      const accounts = await storage.getAllIntegrationAccounts(userId);
      res.json(accounts);
    } catch (error) {
  ...
  app.post("/api/integrations/accounts", requireMaster, async (req, res) => {
    try {
      ...
      const account = await storage.createIntegrationAccount(accountData, credentials);
      res.json(account);
    } catch (error) {
  ...
  app.put("/api/integrations/accounts/:id", requireMaster, async (req, res) => {
    try {
      ...
      const account = await storage.updateIntegrationAccount(req.params.id, updates, credentials);
      if (!account) {
        return res.status(404).json({ error: "账户不存在" });
      }
      res.json(account);
  ...
  app.delete("/api/integrations/accounts/:id", requireMaster, async (req, res) => {
    try {
      const success = await storage.deleteIntegrationAccount(req.params.id);
      if (!success) {
        return res.status(404).json({ error: "账户不存在" });
      }
  ...
  app.post("/api/integrations/accounts/:id/test", requireMaster, async (req, res) => {
    try {
      const result = await storage.getIntegrationAccountWithCredentials(req.params.id);
      if (!result) {
        return res.status(404).json({ error: "账户不存在" });
      }
      
      await storage.updateIntegrationAccount(req.params.id, {
        status: "connected",
  ...
  app.get("/api/integrations/accounts/:accountId/jobs", async (req, res) => {
    try {
      ...
      const jobs = await storage.getAccountSyncJobs(req.params.accountId, limit);
      res.json(jobs);
    } catch (error) {
  ...
  app.post("/api/integrations/accounts/:accountId/sync", requireMaster, async (req, res) => {
    try {
      const job = await storage.createIntegrationSyncJob({
        accountId: req.params.accountId,
        jobType: req.body.jobType || "full_sync",
        status: "pending",
      });
      
      simulateIntegrationSync(job.id, storage);
  ...
  app.post("/api/integrations/wecom/sync-contacts", requireMaster, async (req, res) => {
    try {
      const result = await storage.getIntegrationAccountWithCredentials(req.body.accountId);
      if (!result) {
        return res.status(404).json({ error: "账户不存在" });
      }
      ...
      const existingPersons = await storage.getAllPersons();
  ...
  app.post("/api/integrations/seed-providers", requireMaster, async (req, res) => {
    try {
      const existing = await storage.getAllIntegrationProviders();
      ...
      for (const provider of defaultProviders) {
        await storage.createIntegrationProvider(provider);
      }
