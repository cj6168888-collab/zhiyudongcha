...
class RetrospectionEngineService {
  ...
  async startRetrospection(): Promise<RetrospectionSession> {
    ...
    console.log(`[Retrospection] 复盘引擎启动: ${session.id}`);
    ...
    try {
      ...
      console.log(`[Retrospection] 发现 ${failures.length} 个待处理失败案例`);
      ...
      for (const failure of failures) {
        try {
          ...
          console.error(`[Retrospection] 处理失败案例出错:`, error);
        }
      }
      ...
      console.log(`[Retrospection] 复盘完成: 处理${session.failuresProcessed}个案例, 生成${session.knowledgeGenerated}条知识`);
      
    } catch (error) {
      console.error('[Retrospection] 复盘引擎错误:', error);
  ...
  private async processFailure(failure: CollectedFailure): Promise<{
    ...
    console.log(`[Retrospection] 处理失败案例: ${failure.tagType}`);
  ...
  private async conductLeftRightDebate(failure: CollectedFailure): Promise<DebateRound> {
    console.log(`[Retrospection] 开始左右互搏辩论...`);
