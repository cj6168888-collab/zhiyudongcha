...
class MorningGiftService {
  async generateMorningGift(): Promise<MorningGiftContent> {
    console.log('[MorningGift] 生成晨间礼物...');
    ...
    console.log('[MorningGift] 晨间礼物生成完成');
  ...
  private async collectEvolutionData(from: Date, to: Date): Promise<{
    ...
    const processedFailures = await db.select({ count: count() })
      .from(failureTags)
      .where(and(
    ...
    const newKnowledge = await db.select({ count: count() })
      .from(ragKnowledge)
      .where(gte(ragKnowledge.createdAt, from));
    
    const deployedPatches = await db.select({ count: count() })
      .from(codePatches)
      .where(and(
  ...
  private async identifyRisks(): Promise<RiskItem[]> {
    ...
    const unprocessedFailures = await db.select({ count: count() })
      .from(failureTags)
  ...
  private async prepareSecretWeapons(): Promise<SecretWeapon[]> {
    ...
    const trapKnowledge = await db.select()
      .from(ragKnowledge)
  ...
  private generateGreeting(evolutionData: any): string {
  ...
  private generateEvolutionSummary(evolutionData: any): string {
    const parts: string[] = [];
  ...
  private calculatePerformanceBoost(evolutionData: any): string {
  ...
  private async saveMorningGift(gift: MorningGiftContent): Promise<void> {
    await db.insert(morningGifts).values({
      giftDate: gift.date,
      ...
      opportunities: gift.opportunities as any,
      risks: gift.risks as any,
      secretWeapons: gift.secretWeapons as any,
      todayFocus: gift.todayFocus,
      preparedActions: gift.preparedActions as any,
      performanceBoost: gift.performanceBoost,
