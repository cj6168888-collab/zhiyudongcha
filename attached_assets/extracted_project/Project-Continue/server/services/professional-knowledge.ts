...
class ProfessionalKnowledgeService {
  ...
  async initialize(): Promise<void> {
    if (this.initialized) {
      console.log('[ProfKnowledge] Already initialized, skipping');
      return;
    }
    console.log('[ProfKnowledge] Initializing Professional Knowledge Service...');
    
    try {
      const legalCount = await db.select({ count: sql<number>`count(*)` }).from(legalKnowledge);
      const financeCount = await db.select({ count: sql<number>`count(*)` }).from(financeKnowledge);
      
      const legalNum = Number(legalCount[0]?.count) || 0;
      const financeNum = Number(financeCount[0]?.count) || 0;
      
      console.log(`[ProfKnowledge] Legal entries: ${legalNum}, Finance entries: ${financeNum}`);
      ...
      console.log('[ProfKnowledge] Service initialized successfully');
    } catch (error) {
      console.error('[ProfKnowledge] Initialization error:', error);
    }
  }

  private async seedSampleLegalKnowledge(): Promise<void> {
    console.log('[ProfKnowledge] Seeding comprehensive legal knowledge...');
    ...
    for (const entry of allLegalEntries) {
      try {
        ...
        console.log(`[ProfKnowledge] Skip duplicate: ${entry.lawName} ${entry.articleNumber}`);
      }
    }
    
    console.log(`[ProfKnowledge] Seeded ${seededCount} legal entries from comprehensive database`);
  }

  private async seedSampleFinanceKnowledge(): Promise<void> {
    console.log('[ProfKnowledge] Seeding comprehensive finance knowledge...');
    
    let seededCount = 0;
    for (const entry of extendedFinanceEntries) {
      try {
        const embedding = simpleTextToVector(entry.content, EMBEDDING_DIMENSIONS);
        await db.insert(financeKnowledge).values({
          ...entry,
          embedding: JSON.stringify(embedding),
        });
        seededCount++;
      } catch (error) {
        console.log(`[ProfKnowledge] Skip duplicate finance: ${entry.title}`);
      }
    }
    
    console.log(`[ProfKnowledge] Seeded ${seededCount} finance entries from comprehensive database`);
  }
    ...
    try {
      if (!type || type === 'LEGAL') {
        const legalEntries = category 
          ? await db.select().from(legalKnowledge).where(eq(legalKnowledge.category, category)).limit(100)
          : await db.select().from(legalKnowledge).limit(100);
      ...
      if (!type || type === 'FINANCE') {
        const financeEntries = category
          ? await db.select().from(financeKnowledge).where(eq(financeKnowledge.category, category)).limit(100)
          : await db.select().from(financeKnowledge).limit(100);
      ...
      console.log(`[ProfKnowledge] Search completed in ${processingTime}ms, found ${results.length} matches`);
      
      return results.slice(0, limit);
      
    } catch (error) {
      console.error('[ProfKnowledge] Search error:', error);
      return [];
    }
  }
  ...
  async prepareSyncPackage(
    ...
    if (knowledgeType === 'LEGAL') {
      const entries = await db.select().from(legalKnowledge)
        .where(gte(legalKnowledge.syncVersion, fromVersion));
      items.push(...entries);
    } else {
      const entries = await db.select().from(financeKnowledge)
  ...
  async performContractOCR(imageBase64: string): Promise<OCRResult> {
    ...
    if (!DASHSCOPE_API_KEY) {
      console.warn('[ProfKnowledge] DashScope API未配置，无法执行OCR');
      return {
        text: '',
        confidence: 0,
        language: 'unknown',
...
5. 只输出识别到的文字，不添加任何解释` 
      ...
      console.log(`[ProfKnowledge] OCR completed in ${processingTime}ms, extracted ${extractedText.length} chars`);
      ...
      console.error('[ProfKnowledge] OCR error:', error);
      return {
        text: '',
        confidence: 0,
        language: 'unknown',
  ...
  async analyzeMultiPageContract(imageBase64Array: string[]): Promise<ContractOCRAnalysisResult> {
    ...
    for (let i = 0; i < imageBase64Array.length; i++) {
      console.log(`[ProfKnowledge] Processing page ${i + 1}/${imageBase64Array.length}`);
  ...
  async getStats(): Promise<{
    ...
    const legalEntries = await db.select().from(legalKnowledge);
    const financeEntries = await db.select().from(financeKnowledge);
  ...
  async createPatch(
    ...
    console.log(`[ProfKnowledge] Created patch ${patchId}: ${title}, scheduled for ${scheduledTime.toISOString()}`);
  ...
  async applyPatch(patchId: string): Promise<{ success: boolean; appliedCount: number; errors: string[] }> {
      ...
      try {
        if (item.knowledgeType === 'LEGAL') {
          if (item.operation === 'ADD') {
            ...
            await db.update(legalKnowledge)
          ...
          if (item.operation === 'ADD') {
            ...
            await db.update(financeKnowledge)
              .set({ ...item.data, syncVersion: patch.version })
    ...
    console.log(`[ProfKnowledge] Applied patch ${patchId}: ${appliedCount} items, ${errors.length} errors`);
  ...
  startPatchScheduler(): void {
    ...
    console.log(`[ProfKnowledge] Patch scheduler started, preferred hour: ${this.patchScheduleConfig.preferredHour}:00`);
  }
  ...
  private async checkAndApplyScheduledPatches(): Promise<void> {
    ...
    if (!this.patchScheduleConfig.enableAutoApply) {
      console.log('[ProfKnowledge] Auto-apply disabled, skipping scheduled patches');
      return;
    }
    ...
    console.log(`[ProfKnowledge] Checking scheduled patches: ${duePatches.length} due`);

    for (const patch of duePatches) {
      console.log(`[ProfKnowledge] Applying scheduled patch: ${patch.id}`);
  ...
  updatePatchScheduleConfig(config: Partial<PatchScheduleConfig>): void {
    ...
    console.log(`[ProfKnowledge] Updated patch schedule config:`, this.patchScheduleConfig);
  }
  ...
  skipPatch(patchId: string): boolean {
    ...
    console.log(`[ProfKnowledge] Skipped patch ${patchId}`);
    return true;
  }
  ...
  async seedExtendedKnowledge(): Promise<{ legal: number; finance: number }> {
    console.log('[ProfKnowledge] Starting extended knowledge seed...');
    ...
    for (const entry of allLegalEntries) {
      try {
        ...
        if (entry.lawName && entry.articleNumber) {
          existing = await db.select().from(legalKnowledge)
            .where(and(
        ...
        console.error(`[ProfKnowledge] Failed to insert legal entry: ${entry.lawName} ${entry.articleNumber}`, error);
      }
    }
    ...
    for (const entry of extendedFinanceEntries) {
      try {
        let existing;
        if (entry.title) {
          existing = await db.select().from(financeKnowledge)
        ...
        console.error(`[ProfKnowledge] Failed to insert finance entry: ${entry.title}`, error);
      }
    }

    console.log(`[ProfKnowledge] Extended seed complete: ${legalInserted} legal, ${financeInserted} finance entries`);
  ...
  async getKnowledgeStats(): Promise<{ 
    ...
    const legalResult = await db.select({ count: sql<number>`count(*)` }).from(legalKnowledge);
    const financeResult = await db.select({ count: sql<number>`count(*)` }).from(financeKnowledge);
    
    const legalCats = await db.select({ 
      category: legalKnowledge.category,
      count: sql<number>`count(*)`
    }).from(legalKnowledge).groupBy(legalKnowledge.category);
    
    const financeCats = await db.select({ 
