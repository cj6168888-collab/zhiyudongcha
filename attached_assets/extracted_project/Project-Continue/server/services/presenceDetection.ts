...
export function processVoiceEvent(
  ...
  console.log(`[PresenceDetection] 处理声音事件`);
  
  if (!masterVoiceTemplate || masterVoiceTemplate.length === 0) {
    console.log(`[PresenceDetection] 无主人声纹模板，保持当前模式`);
  ...
  if (verificationResult.isMatch) {
    console.log(`[PresenceDetection] 主人声纹验证通过，置信度: ${(verificationResult.confidence * 100).toFixed(1)}%`);
    ...
    console.log(`[PresenceDetection] 检测到陌生声纹，置信度: ${(verificationResult.confidence * 100).toFixed(1)}%`);
...
export function processDeviceEvent(event: DeviceEvent): { newMode: InteractionMode; transition?: ModeTransition } {
  console.log(`[PresenceDetection] 处理设备事件: ${event.type}`);
  ...
  switch (event.type) {
    ...
    case 'NETWORK_CHANGE':
      console.log(`[PresenceDetection] 位置/网络变化，重置访客检测状态`);
      currentState.guestDetected = false;
      break;
  }
...
export function triggerEmergencyMode(reason: string): ModeTransition {
  console.log(`[PresenceDetection] ⚠️ 紧急模式触发: ${reason}`);
...
export function clearEmergencyMode(masterConfirmed: boolean): { newMode: InteractionMode; transition?: ModeTransition } {
  if (!masterConfirmed) {
    console.log(`[PresenceDetection] 需要主人确认才能解除紧急状态`);
    return { newMode: currentState.currentMode };
  }
  
  console.log(`[PresenceDetection] 主人确认，解除紧急状态`);
  currentState.emergencyTriggered = false;
...
export function clearGuestPresence(): { newMode: InteractionMode; transition?: ModeTransition } {
  console.log(`[PresenceDetection] 清除访客状态`);
  currentState.guestDetected = false;
...
function transitionToMode(
  ...
  console.log(`[PresenceDetection] 模式切换: ${oldMode} → ${newMode} (${reason})`);
...
export function resetPresenceState(): void {
  ...
  console.log(`[PresenceDetection] 存在状态已重置`);
}

console.log('[PresenceDetection] 存在检测管道已加载');
