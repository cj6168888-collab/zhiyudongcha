...
export function configureAutoGLM(config: AutoGLMConfig): void {
  ...
  console.log(`[ModelRouter] AutoGLM configured: ${config.endpoint} (${config.model})`);
}
...
export function addServerConfig(config: ServerConfig): void {
  ...
  console.log(`[ModelRouter] Server configured: ${config.name || config.endpoint} (${config.model})`);
  if (config.gpuMemoryMB) {
    console.log(`[ModelRouter] GPU: ${config.gpuMemoryMB}MB, RAM: ${config.ramMB}MB`);
  }
}
...
export async function callServerModel(
  ...
  try {
    ...
    if (!response.ok) {
      console.log(`[ModelRouter] Server ${server.endpoint} returned ${response.status}`);
      return null;
    }
    ...
    console.log(`[ModelRouter] Server error (${server.endpoint}): ${error instanceof Error ? error.message : 'Unknown'}`);
    return null;
  }
}
...
export async function callLocalModel(
  ...
  try {
    ...
    if (!result) {
      console.log('[ModelRouter] Local model returned no result');
      return null;
    }
    ...
    console.log(`[ModelRouter] Local model error: ${error instanceof Error ? error.message : 'Unknown error'}`);
    return null;
  }
}
...
export async function routeToModel(
  ...
  if (localFast.canHandle && localFast.response) {
    console.log(`[ModelRouter] Using local fast response for: ${localFast.category}`);
    return {
  ...
  if (isScreenOperationTask(userMessage) && autoGLMConfig?.enabled) {
    console.log('[ModelRouter] Detected screen operation task, routing to AutoGLM');
    return {
  ...
  console.log(`[ModelRouter] Task complexity: ${complexity.level} (${complexity.score.toFixed(2)}) - ${complexity.reason}`);
  ...
  if (shouldTryServer) {
    ...
    if (bestServer) {
      console.log(`[ModelRouter] Routing COMPLEX task to GPU server: ${bestServer.name || bestServer.endpoint}`);
      const serverResponse = await callServerModel(messages, userMessage, bestServer);
      if (serverResponse) {
        console.log(`[ModelRouter] Server responded in ${serverResponse.latencyMs}ms (${bestServer.model})`);
        return serverResponse;
      }
      console.log('[ModelRouter] Server unavailable, falling back to local-first path...');
    }
  }
  ...
  if (useLocal) {
    ...
    if (localResponse) {
      console.log(`[ModelRouter] Local model responded in ${localResponse.latencyMs}ms`);
    ...
    console.log('[ModelRouter] Falling back to cloud model');
  }
  ...
  try {
    ...
    console.error('[ModelRouter] Cloud model error:', error);
