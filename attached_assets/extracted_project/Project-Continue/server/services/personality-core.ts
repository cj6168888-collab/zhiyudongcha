...
export class PersonalityCoreService {
  ...
  async initializeOriginMaster(masterId: string): Promise<LoyaltyAuthorization> {
    ...
    const [result] = await db.insert(loyaltyAuthorizations).values({
  ...
  async switchPersona(newPersona: PersonaType, reason?: string): Promise<PersonalityContext> {
    ...
    const [result] = await db.insert(personalityStates).values({
  ...
  async updateEmotionState(
    ...
    await db.insert(personalityStates).values({
      activePersona: currentState.activePersona,
  ...
  async assessCapability(
    ...
    // 记录评估结果
    const [assessment] = await db.insert(capabilityAssessments).values({
      taskDescription,
      taskType,
  ...
  async createPromise(
    ...
    const [promise] = await db.insert(promiseTracking).values({
...
无聊 → "爸爸想和小智玩吗？人家可以陪你聊天呀～"`;
  ...
  async searchIdentity(query: {
    ...
    if (query.voiceprint) {
      const all = await db.select().from(socialIdentityIndex);
  ...
  async registerSocialIdentity(
    ...
    const [result] = await db.insert(socialIdentityIndex).values({
      recognizedName,
