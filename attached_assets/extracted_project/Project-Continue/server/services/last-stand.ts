...
class LastStandService {
  ...
  constructor() {
    ...
    console.log('[LastStand] 最后防线协议已初始化');
  }
    ...
    for (const trigger of this.voiceTriggers) {
      ...
      if (similarity >= trigger.similarity * config.sensitivity) {
        ...
        if (trigger.requireVoiceprint && !voiceprintMatch) {
          console.log('[LastStand] 语音触发但声纹未验证');
          continue;
        }
        ...
        console.log(`[LastStand] 语音触发激活: "${trigger.phrase}" (相似度: ${similarity.toFixed(2)})`);
  ...
  async checkBiometricTrigger(metrics: {
    ...
    if (reasons.length >= 2 || (metrics.hostileVoiceDetected && reasons.length >= 1)) {
      ...
      console.log(`[LastStand] 生物特征触发: ${reasons.join(', ')}`);
  ...
  async checkRemoteTrigger(command: {
    ...
    if (!device) {
      console.log(`[LastStand] 远程触发失败: 未知设备 ${command.deviceId}`);
    ...
    if (!masterSecret || masterSecret.length < 32) {
      console.error('[LastStand] 安全错误: AVATAR_MASTER_SECRET 未设置或过短');
      return { triggered: false, authorized: false };
    }
    const parts = command.authToken.split(':');
    
    if (parts.length !== 2) {
      console.log('[LastStand] 远程触发失败: 令牌格式无效');
      return { triggered: false, authorized: false };
    }
    ...
    if (isNaN(timestampNum) || Math.abs(now - timestampNum) > 5 * 60 * 1000) {
      console.log('[LastStand] 远程触发失败: 令牌已过期');
      return { triggered: false, authorized: false };
    }
    ...
    if (!isAuthorized) {
      console.log('[LastStand] 远程触发失败: 令牌签名验证失败');
      return { triggered: false, authorized: false };
    }

    console.log(`[LastStand] 远程触发激活: 设备=${command.deviceId}, 等级=${command.level}`);
  ...
  async checkGeoFenceTrigger(location: {
    ...
    for (const fence of this.geoFences) {
      ...
      if (distance <= fence.radius) {
        console.log(`[LastStand] 地理围栏触发: ${fence.name} (距离: ${distance.toFixed(0)}m)`);
  ...
  async checkDeadmanSwitch(): Promise<{
    ...
    if (now > deadline) {
      ...
      console.log(`[LastStand] 死人开关触发: 逾期 ${hoursOverdue.toFixed(1)} 小时`);
  ...
  async executeMeltdown(
    ...
    try {
      console.log(`[LastStand] 开始熔断协议: 等级=${level}, 触发=${triggerType}`);
      ...
      console.log(`[LastStand] 熔断协议完成: ${report.filesDestroyed}文件销毁, ${report.cachesCleared}缓存清除`);

      return report;

    } catch (error) {
      this.state = 'STANDBY';
      console.error('[LastStand] 熔断协议执行失败:', error);
      throw error;
    }
  }

  private async executeVisualWipe(): Promise<void> {
    console.log('[LastStand] 步骤1: 视觉清空');
    ...
    for (const device of devices) {
      if (device.ws && device.ws.readyState === 1) {
        try {
          ...
          console.log(`[LastStand] 发送清空指令到 ${device.id} 失败`);
        }
      }
    }
  }
  ...
  private async executeLocalMeltdown(level: MeltdownLevel): Promise<{
    ...
    console.log(`[LastStand] 步骤2: 本地熔断 (等级: ${level})`);
    ...
    for (const [priority, categories] of Array.from(priorityGroups.entries())) {
      console.log(`[LastStand] 销毁优先级 ${priority} 数据: ${categories.join(', ')}`);
  ...
  private async executeSecureBackup(): Promise<boolean> {
    console.log('[LastStand] 步骤3: 安全归位 (加密备份)');
    ...
    try {
      ...
      console.log(`[LastStand] 备份完成: ${shards.length}个分片`);
      return true;

    } catch (error) {
      console.error('[LastStand] 备份失败:', error);
      return false;
    }
  }

  private async injectDecoyData(): Promise<void> {
    console.log('[LastStand] 焦土策略: 注入诱饵数据');
    ...
    for (const decoy of decoys) {
      console.log(`[LastStand] 注入诱饵: ${decoy.type}`);
    }
  }
  ...
  async initiateRecovery(secondaryAuth: {
    ...
    console.log(`[LastStand] 启动复活协议: 验证方式=${secondaryAuth.method}`);
  ...
  async executeRecovery(tokenString: string): Promise<{
    ...
    try {
      ...
      console.log(`[LastStand] 复活完成: 恢复 ${restoredCount} 项数据`);
      ...
      console.error('[LastStand] 恢复失败:', error);
      return {
        success: false,
        restoredItems: 0,
        message: '数据恢复过程中出现错误',
  ...
  setVoiceTriggers(triggers: VoiceTrigger[]): void {
    ...
    console.log(`[LastStand] 语音触发词已更新: ${triggers.length}个`);
  }
  ...
  setGeoFences(fences: GeoFence[]): void {
    ...
    console.log(`[LastStand] 地理围栏已更新: ${fences.length}个`);
  }

  configureDeadmanSwitch(config: Partial<DeadmanSwitch>): void {
    this.deadmanSwitch = { ...this.deadmanSwitch, ...config };
    console.log(`[LastStand] 死人开关配置已更新: 启用=${this.deadmanSwitch.enabled}`);
  }
  ...
  private async destroyCategory(category: string): Promise<{ files: number; caches: number }> {
    ...
    console.log(`[LastStand] 销毁 ${category}: ${filesDestroyed}文件, ${cachesCleared}缓存`);
  ...
  private getAllDeviceIds(): string[] {
    return deviceRegistry.getOnlineDevices().map((d: any) => d.id);
  }
  ...
  private async sendHapticAlert(): Promise<void> {
    try {
      ...
      console.log('[LastStand] 触觉警报发送失败');
    }
  }
  ...
  private async logMeltdown(report: MeltdownReport): Promise<void> {
    try {
      ...
      console.error('[LastStand] 审计日志写入失败');
    }
  }
}
