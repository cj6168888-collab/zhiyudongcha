...
class SpiritSingletonService {
  ...
  constructor() {
    ...
    console.log('[SpiritSingleton] 灵魂单例服务已启动（含唯一性控制层）');
    console.log(`[SpiritSingleton] 心跳检测: ${HEARTBEAT_INTERVAL_MS}ms, 超时: ${HEARTBEAT_TIMEOUT_MS}ms`);
  }
  ...
  private async initializeDatabaseMutex(): Promise<void> {
    try {
      const existing = await db.select().from(globalMutex).where(eq(globalMutex.mutexKey, MUTEX_KEY)).limit(1);
      
      if (existing.length === 0) {
        await db.insert(globalMutex).values({
          mutexKey: MUTEX_KEY,
          fencingToken: 0,
          migrationState: 'IDLE',
          avatarPoseJson: this.token.avatarPose,
        });
        console.log('[SpiritSingleton] 数据库互斥锁已创建');
      } else {
        // 从数据库恢复状态
        ...
        console.log(`[SpiritSingleton] 从数据库恢复状态，fencingToken: ${this.fencingToken}`);
      }
    } catch (error) {
      console.error('[SpiritSingleton] 初始化数据库互斥锁失败:', error);
    }
  }
  ...
  private async syncToDatabase(): Promise<void> {
    try {
      ...
      console.error('[SpiritSingleton] 同步到数据库失败:', error);
    }
  }
  ...
  private async checkHeartbeatTimeouts(): Promise<void> {
    ...
    try {
      // 从数据库查找超时的设备
      const timedOutDevices = await db.select()
      ...
      for (const device of timedOutDevices) {
        ...
        if (this.token.activeDeviceId === device.deviceId) {
          console.log(`[SpiritSingleton] 设备心跳超时，自动释放: ${device.deviceId}`);
      ...
      console.error('[SpiritSingleton] 心跳检查失败:', error);
    }
  }
  ...
  async handleHeartbeat(deviceId: string): Promise<{ 
    ...
    try {
      ...
      console.error('[SpiritSingleton] 更新心跳失败:', error);
    }
  ...
  private async updateMigrationState(
    ...
    try {
      ...
      console.log(`[SpiritSingleton] 迁移状态更新: ${state}`);
    } catch (error) {
      console.error('[SpiritSingleton] 更新迁移状态失败:', error);
    }
  }
  ...
  private async requestMigrationWithKill(targetDeviceId: string): Promise<MigrationEvent> {
    ...
    try {
      ...
      console.error('[SpiritSingleton] 记录迁移日志失败:', error);
    }
    ...
    console.log(`[SpiritSingleton] 发送 Kill_Active_UI: ${sourceDeviceId}`);
    ...
    console.log(`[SpiritSingleton] 迁移完成: ${migrationId}, fencingToken: ${this.fencingToken}`);
    return migration;
  }
  ...
  private waitForKillAckWithResult(
    ...
    return new Promise((resolve) => {
      const timeout = setTimeout(() => {
        ...
        console.log(`[SpiritSingleton] Kill确认超时: ${deviceId}, migrationId: ${migrationId}`);
        resolve(false);
      }, timeoutMs);
  ...
  async acknowledgeKill(deviceId: string, migrationId?: string): Promise<void> {
    ...
    if (pending) {
      ...
      console.log(`[SpiritSingleton] 收到 Kill_ACK: ${deviceId}, migrationId: ${migrationId || 'unknown'}`);
    }
  }
  ...
  requestAudioStream(deviceId: string): { 
    ...
    if (this.audioStreamDevice && this.audioStreamDevice !== deviceId) {
      ...
      console.log(`[SpiritSingleton] 强制终止音频流: ${this.audioStreamDevice}`);
    }
  ...
  releaseAudioStream(deviceId: string): void {
    if (this.audioStreamDevice === deviceId) {
      ...
      this.syncToDatabase();
      console.log(`[SpiritSingleton] 音频流已释放: ${deviceId}`);
    }
  }
  ...
  private emit(event: string, data: any): void {
    this.listeners.forEach(listener => {
      try {
        ...
        console.error('[SpiritSingleton] Listener error:', e);
      }
    });
  }
  ...
  async registerDevice(info: DevicePresenceInfo): Promise<{ success: boolean; isActive: boolean; fencingToken: number }> {
    ...
    try {
      ...
      console.error('[SpiritSingleton] 设备注册到数据库失败:', error);
    }
    
    console.log(`[SpiritSingleton] 设备注册: ${info.deviceName} (${info.deviceType})`);
  ...
  unregisterDevice(deviceId: string): void {
    ...
    console.log(`[SpiritSingleton] 设备注销: ${deviceId}`);
  ...
  private checkTokenExpiry(): void {
    if (this.token.activeDeviceId && Date.now() > this.token.expiresAt) {
      console.log(`[SpiritSingleton] 令牌过期: ${this.token.activeDeviceId}`);
  ...
  private acquireToken(deviceId: string): boolean {
    if (this.token.migrationInProgress) {
      console.log('[SpiritSingleton] 迁移进行中，无法获取令牌');
      return false;
    }
    ...
    console.log(`[SpiritSingleton] 令牌授予: ${deviceId}`);
  ...
  async requestMigration(
    ...
    console.log(`[SpiritSingleton] 开始迁移: ${sourceDeviceId || 'null'} → ${targetDeviceId}`);
    ...
    console.log(`[SpiritSingleton] 迁移完成: ${targetDeviceId} (${migration.endTime - migration.startTime}ms)`);
  ...
  forceHibernate(deviceId: string): void {
    ...
    console.log(`[SpiritSingleton] 强制休眠: ${deviceId}`);
  }
  ...
  setAvatarMode(mode: AvatarMode): void {
    ...
    console.log(`[SpiritSingleton] 模式切换: ${mode}`);
  }
  ...
  private checkInactiveDevices(): void {
    ...
    for (const [deviceId, device] of this.devices.entries()) {
      if (now - device.lastActivity > INACTIVITY_THRESHOLD_MS) {
        ...
        if (this.token.activeDeviceId === deviceId) {
          console.log(`[SpiritSingleton] 活跃设备超时: ${deviceId}`);
          const nextDevice = this.selectBestDevice();
