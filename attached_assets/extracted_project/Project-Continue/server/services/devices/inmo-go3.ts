...
class INMOGo3Driver {
  ...
  async connect(): Promise<boolean> {
    console.log(`[INMO Go3] 正在连接设备: ${this.config.deviceName}`);
    ...
    console.log(`[INMO Go3] 设备已连接: ${this.config.deviceName}`);
    
    return true;
  }

  async disconnect(): Promise<void> {
    console.log(`[INMO Go3] 正在断开设备: ${this.config.deviceName}`);
  ...
  async displayMessage(message: INMODisplayMessage): Promise<boolean> {
    if (!this.status.isConnected) {
      console.warn('[INMO Go3] 设备未连接，无法显示消息');
      return false;
    }

    console.log(`[INMO Go3] 显示消息: ${message.type} - ${message.content.substring(0, 50)}...`);
  ...
  async setBrightness(level: number): Promise<void> {
    ...
    console.log(`[INMO Go3] 亮度设置为: ${this.config.displayBrightness}%`);
  }
  ...
  async startRealtimeTranslation(sourceLang: string, targetLang: string): Promise<void> {
    ...
    console.log(`[INMO Go3] 启动实时翻译: ${sourceLang} -> ${targetLang}`);
...
class INMODeviceManager {
  ...
  async registerDevice(config: Partial<INMOGo3Config>): Promise<INMOGo3Driver> {
    ...
    console.log(`[INMO Manager] 设备已注册: ${deviceId}`);
  ...
  async removeDevice(deviceId: string): Promise<boolean> {
    ...
    if (device) {
      ...
      console.log(`[INMO Manager] 设备已移除: ${deviceId}`);
      return true;
    }
    return false;
  }
