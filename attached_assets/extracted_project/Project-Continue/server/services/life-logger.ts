...
class LifeLoggerService {
  ...
  async recordHealthData(input: HealthDataInput): Promise<HealthMetrics> {
    ...
    console.log(`[LifeLogger] 健康数据已记录: HR=${input.heartRate}, Steps=${input.steps}`);
  ...
  async recordSocialInteraction(input: SocialInteractionInput): Promise<SocialInteraction> {
    ...
    const [result] = await db.insert(socialInteractions).values(data).returning();
    this.lastSocialSync = new Date();
    
    console.log(`[LifeLogger] 社交互动已记录: ${input.interactionType} with ${input.contactAlias || 'unknown'}`);
    
    return result;
  }
  
  async recordBehaviorPattern(input: InsertBehaviorPattern): Promise<BehaviorPattern> {
    const [result] = await db.insert(behaviorPatterns).values(input).returning();
    console.log(`[LifeLogger] 行为模式已记录: ${input.patternType}`);
    return result;
  }
  ...
  async generateDailyReport(date: Date = new Date()): Promise<DailyEnergyReport> {
    ...
    if (existing.length > 0) {
      const [result] = await db.update(dailyEnergyReports)
        .set(reportData)
        .where(eq(dailyEnergyReports.id, existing[0].id))
        .returning();
      console.log(`[LifeLogger] 每日报告已更新: ${reportDate.toISOString().split('T')[0]}`);
      return result;
    } else {
      const [result] = await db.insert(dailyEnergyReports).values(reportData).returning();
      console.log(`[LifeLogger] 每日报告已生成: ${reportDate.toISOString().split('T')[0]}`);
      return result;
    }
  }
