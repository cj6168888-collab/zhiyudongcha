...
class ResourceManagerService {
  ...
  registerDevice(device: DeviceCapability): void {
    ...
    console.log(`[ResourceManager] Device registered: ${device.deviceName} (${device.deviceType})`);
  }
  ...
  assignTask(task: TaskRequirement): TaskAssignment | null {
    ...
    if (candidates.length === 0) {
      console.log(`[ResourceManager] No suitable device for task ${task.taskId}`);
      return null;
    }
    ...
    console.log(`[ResourceManager] Task ${task.taskId} assigned to ${best.device.deviceName}`);
  ...
  checkDeviceHealth(): void {
    ...
    for (const device of Array.from(this.devices.values())) {
      if (now - device.lastHeartbeat.getTime() > timeout) {
        ...
        console.log(`[ResourceManager] Device offline: ${device.deviceName}`);
      }
    }
  }
