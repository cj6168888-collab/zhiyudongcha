...
class DeviceSentryService {
  ...
  private analysisCallbacks: ((data: any) => void)[] = [];
  ...
  async startSentry(mode: SentryMode = 'FULL_SENSE'): Promise<void> {
    if (this.sentryActive) {
      console.log('[DeviceSentry] Already active, changing mode...');
    }
    
    this.mode = mode;
    this.sentryActive = true;
    
    console.log(`[DeviceSentry] Started in ${mode} mode`);
  ...
  async stopSentry(): Promise<void> {
    ...
    console.log('[DeviceSentry] Stopped');
  ...
  registerDevice(device: DeviceConnection): void {
    ...
    console.log(`[DeviceSentry] Device registered: ${device.deviceId} (${device.deviceType})`);
  }
  
  unregisterDevice(deviceId: string): void {
    this.connectedDevices.delete(deviceId);
    console.log(`[DeviceSentry] Device unregistered: ${deviceId}`);
  }
  ...
  async createProactiveAlert(alert: Omit<ProactiveAlert, 'id' | 'createdAt' | 'acknowledged'>): Promise<ProactiveAlert> {
    ...
    console.log(`[DeviceSentry] Alert created: ${fullAlert.type} - ${fullAlert.title}`);
  ...
  private notifyAnalysisCallbacks(data: any): void {
    for (const callback of this.analysisCallbacks) {
      try {
        callback(data);
      } catch (error) {
        console.error('[DeviceSentry] Callback error:', error);
      }
    }
  }
  ...
  async sendHapticFeedback(deviceId: string, pattern: ProactiveAlert['hapticPattern']): Promise<boolean> {
    ...
    console.log(`[DeviceSentry] Sending haptic feedback to ${deviceId}: ${pattern}`);
    return true;
  }
  ...
  setStealthMode(enabled: boolean): void {
    ...
    console.log(`[DeviceSentry] Stealth mode: ${enabled ? 'ON' : 'OFF'}`);
  }
}
