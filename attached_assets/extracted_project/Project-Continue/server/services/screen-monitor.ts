    ...
    const timer = setTimeout(async () => {
      ...
      if (state && !state.analysisInProgress) {
        ...
        try {
          ...
          state.analysisInProgress = false;
          console.error('[Screen Monitor] Analysis error:', error);
        }
      }
...
export function startVisionSniffing(
  ...
  const sniffingLoop = async () => {
    ...
    try {
      ...
      console.error('[Vision Sniffing] Capture error:', error);
    }
  };
  ...
  console.log(`[Vision Sniffing] Started for session ${sessionId}, interval: ${config.sniffingInterval}ms`);
}
...
export function stopVisionSniffing(sessionId: string): void {
  ...
  if (state) {
    ...
    console.log(`[Vision Sniffing] Stopped for session ${sessionId}`);
  }
}
...
function triggerZ3Alert(sessionId: string, analysis: ScreenAnalysis): void {
  ...
  console.log(`[Z3 Whisper] Alert triggered for ${sessionId}: ${analysis.alertLevel} - ${analysis.type}`);
  ...
  alertCallbacks.forEach(callback => {
    try {
      ...
      console.error('[Z3 Whisper] Alert callback error:', error);
    }
  });
}
