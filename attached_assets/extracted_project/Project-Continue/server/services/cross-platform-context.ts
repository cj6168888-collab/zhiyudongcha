...
function parseJSONResponse(text: string): any {
  ...
  if (jsonMatch) {
    try {
      ...
      console.error('[CrossPlatformContext] Failed to parse JSON:', e);
      return null;
    }
  }
  return null;
  ...
  async analyzeContactContext(contactId: string): Promise<ContextAnalysis | null> {
    ...
    try {
      ...
      return {
        ...
        pendingItems: (parsed.pendingItems || []).map((p: any, i: number) => ({
        ...
        suggestedReminders: (parsed.suggestedReminders || []).map((r: any) => ({
      ...
      console.error('[CrossPlatformContext] Analysis failed:', error);
      return this.createFallbackAnalysis(contactId, context, messages);
    }
  }
