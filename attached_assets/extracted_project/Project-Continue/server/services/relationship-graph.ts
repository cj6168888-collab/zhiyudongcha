...
class RelationshipGraphService {
  async createEdge(input: InsertRelationshipEdge): Promise<RelationshipEdge> {
    ...
    const [edge] = await db.insert(relationshipEdges)
      .values(input)
      .returning();
    
    console.log(`[RelationshipGraph] Edge created: ${input.fromPersonId} -> ${input.toPersonId}`);
    return edge;
  }
  ...
  async updateEdge(id: string, updates: Partial<InsertRelationshipEdge>): Promise<RelationshipEdge> {
    const [edge] = await db.update(relationshipEdges)
      .set({
        ...updates,
        updatedAt: new Date(),
      })
  ...
  async deleteEdge(id: string): Promise<void> {
    await db.delete(relationshipEdges)
      .where(eq(relationshipEdges.id, id));
    
    console.log(`[RelationshipGraph] Edge deleted: ${id}`);
  }
  ...
  async createSuggestion(input: InsertRelationshipSuggestion): Promise<RelationshipSuggestion> {
    const [suggestion] = await db.insert(relationshipSuggestions)
      .values(input)
      .returning();
    
    console.log(`[RelationshipGraph] Suggestion created: ${suggestion.suggestionType} for person ${input.personId}`);
    return suggestion;
  }
  ...
  async dismissSuggestion(id: string): Promise<void> {
    await db.update(relationshipSuggestions)
      .set({
        status: 'DISMISSED',
  ...
  async generateContactReminders(): Promise<RelationshipSuggestion[]> {
    ...
    const staleEdges = await db.select()
      .from(relationshipEdges)
      .where(
        and(
          eq(relationshipEdges.isActive, true),
    ...
    console.log(`[RelationshipGraph] Generated ${suggestions.length} contact reminders`);
    return suggestions;
  }
