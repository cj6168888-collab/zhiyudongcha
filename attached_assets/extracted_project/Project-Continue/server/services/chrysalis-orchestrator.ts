...
class ChrysalisOrchestratorService {
  ...
  private async checkAndStartNightCycle(): Promise<void> {
    ...
    if (this.NIGHT_CYCLE_HOURS.includes(hour) && !this.isNightCycleActive) {
      console.log('[Chrysalis] 检测到夜间时段，启动进化周期...');
      this.runFullCycle().catch(err => {
        console.error('[Chrysalis] 夜间周期错误:', err);
      });
    }
  }
  ...
  async runFullCycle(): Promise<ChrysalisCycleResult> {
    ...
    console.log(`[Chrysalis] 开始完整进化周期: ${cycleId}`);
    ...
    try {
      ...
      console.log('[Chrysalis] Phase 1: 失败标签收集');
      ...
      console.log('[Chrysalis] Phase 2: 复盘引擎');
      ...
      console.log('[Chrysalis] Phase 3: 逻辑微调');
      ...
      console.log('[Chrysalis] Phase 6: 生成晨间礼物');
      ...
      console.error('[Chrysalis] 进化周期出错:', error);
    } finally {
    ...
    console.log(`[Chrysalis] 进化周期完成: ${cycleId}, 耗时${durationMs}ms`);
  ...
  async runPartialCycle(phases: ChrysalisPhase[]): Promise<Partial<ChrysalisCycleResult>> {
    console.log(`[Chrysalis] 运行部分周期: ${phases.join(', ')}`);
    ...
    for (const phase of phases) {
      try {
        ...
        console.error(`[Chrysalis] Phase ${phase} 失败:`, error);
      }
    }
