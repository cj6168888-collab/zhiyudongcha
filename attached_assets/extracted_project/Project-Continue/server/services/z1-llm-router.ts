...
export function configureZ1Router(config: Partial<Z1RouterConfig>): void {
  ...
  console.log('[Z1Router] Configuration updated:', {
...
async function getUserPreferences(userId?: string): Promise<{
  ...
  try {
    const prefs = await db.select().from(userRoutingPreferences).where(eq(userRoutingPreferences.userId, userId)).limit(1);
    if (prefs.length > 0) {
      return {
    ...
    console.log('[Z1Router] Failed to fetch user preferences:', error);
  }
...
async function logRoutingDecision(
  ...
  try {
    const [log] = await db.insert(routingLogs).values({
    ...
    console.log('[Z1Router] Failed to log routing decision:', error);
    return undefined;
  }
}
...
export async function routeZ1(
  ...
  console.log(`[Z1Router] Classification in ${classification.classificationTimeMs}ms:`, {
    taskType: classification.taskType,
  ...
  for (const providerName of attemptOrder) {
    ...
    if (!available) {
      console.log(`[Z1Router] Provider ${providerName} not available, trying next...`);
    ...
    if (response) {
      ...
      console.log(`[Z1Router] Success with ${providerName} (${model}) in ${totalLatency}ms`);
    ...
    console.log(`[Z1Router] Provider ${providerName} failed, trying next...`);
    fallbackUsed = true;
  }
...
export async function *routeZ1Stream(
  ...
  for (const providerName of attemptOrder) {
    ...
    try {
      ...
      console.log(`[Z1Router] Stream failed for ${providerName}:`, error);
    }
  }
}
...
export async function getRoutingLogs(
  ...
  try {
    let query = db.select().from(routingLogs).limit(limit).orderBy(routingLogs.createdAt);
    if (userId) {
      return await db.select().from(routingLogs).where(eq(routingLogs.userId, userId)).limit(limit);
    }
    return await query;
  } catch (error) {
    console.log('[Z1Router] Failed to fetch routing logs:', error);
    return [];
  }
}
...
export async function updateUserRoutingPreferences(
  ...
  try {
    const existing = await db.select().from(userRoutingPreferences).where(eq(userRoutingPreferences.userId, userId)).limit(1);
    ...
    console.log('[Z1Router] Failed to update user preferences:', error);
    return false;
  }
}

console.log('[Z1Router] 智能路由层 v2.1 已初始化 (Phase 1.2 完整版)');
