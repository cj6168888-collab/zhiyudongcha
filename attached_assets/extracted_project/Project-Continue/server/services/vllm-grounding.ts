...
class VLLMGroundingService {
  ...
  setProvider(provider: VLLMProvider | string, config?: Partial<VLLMConfig>): void {
    if (provider === 'OPENAI' || provider.toLowerCase().includes('openai')) {
      console.error('[VLLMGrounding] Z1 协议违规：OPENAI provider 被禁止，使用 DASHSCOPE 替代');
      provider = 'DASHSCOPE';
    }
    ...
    this.adapter = this.createAdapter();
    console.log(`[VLLMGrounding] Provider switched to: ${validProvider}`);
  }
  ...
  async analyzeScreen(image: string): Promise<ScreenAnalysis> {
    ...
    try {
      ...
      console.error('[VLLMGrounding] Screen analysis failed:', error);
      return {
        elements: [],
        layout: {
          screenWidth: 1080,
  ...
  private parseGroundingResponse(
    ...
    try {
      ...
      console.error('[VLLMGrounding] Parse error:', error);
      return {
        found: false,
        confidence: 0,
        target,
  ...
  private parseScreenAnalysis(response: string, startTime: number): ScreenAnalysis {
    try {
      ...
      const elements: AnalyzedElement[] = (data.elements || []).map((e: any, i: number) => ({
        id: e.id ?? `elem_${i}`,
        type: e.type ?? 'unknown',
        label: e.label ?? '',
      ...
      console.error('[VLLMGrounding] Screen analysis parse error:', error);
      return {
        elements: [],
        layout: {
          screenWidth: 1080,
...
class TaskSessionTracker {
  ...
  startSession(taskDescription: string): TaskSession {
    ...
    console.log(`[TaskSession] 开始任务: ${taskDescription} (${session.id})`);
    return session;
  }
  ...
  recordStep(
    ...
    console.log(`[TaskSession] Step ${step.id}: ${action} - ${success ? '✓' : '✗'} (${latencyMs}ms, 效率: ${session.efficiency}%)`);
    return step;
  }
  ...
  completeSession(sessionId: string, success: boolean): TaskSession | null {
    ...
    console.log(`[TaskSession] 任务${success ? '完成' : '失败'}: ${session.taskDescription}`);
    console.log(`[TaskSession] 统计: ${session.totalSteps}步, ${session.successfulSteps}成功, 耗时${duration}ms, 效率${session.efficiency}%`);
...
const initVLLMGrounding = (): VLLMGroundingService => {
  ...
  if (dashscopeApiKey) {
    console.log('[VLLMGrounding] Initializing with DashScope provider');
  ...
  // 检查是否有本地 Ollama
  console.log('[VLLMGrounding] No API key found, using MOCK provider');
  return new VLLMGroundingService({
    provider: 'MOCK',
  });
};
