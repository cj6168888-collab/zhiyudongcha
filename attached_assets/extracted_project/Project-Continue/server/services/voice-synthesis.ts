...
export async function synthesizeSpeech(
  ...
  if (!DASHSCOPE_API_KEY) {
    ...
    if (isAzureConfigured()) {
      console.log('[Voice Synthesis] DashScope not configured, falling back to Azure');
  ...
  try {
    ...
    if (!response.ok) {
      ...
      const { synthesizeWithAzure, isAzureConfigured } = await import('./azure-tts');
      if (isAzureConfigured()) {
        console.log('[Voice Synthesis] DashScope failed, falling back to Azure');
        const result = await synthesizeWithAzure(text, {
          voice: 'xiaoxiao',
          rate: options.rate,
          pitch: options.pitch,
    ...
    const { synthesizeWithAzure, isAzureConfigured } = await import('./azure-tts');
    if (isAzureConfigured()) {
      console.log('[Voice Synthesis] DashScope network error, falling back to Azure');
      const result = await synthesizeWithAzure(text, {
        voice: 'xiaoxiao',
        rate: options.rate,
        pitch: options.pitch,
