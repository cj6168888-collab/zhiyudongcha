...
class RiskPredictionService {
  ...
  private async assessResignationRisk(memberId: string, member: any): Promise<{
    type: RiskType;
    level: RiskLevel;
    score: number;
    indicators: RiskIndicator[];
  ...
  private async assessLeakRisk(memberId: string, member: any): Promise<{
    type: RiskType;
    level: RiskLevel;
    score: number;
    indicators: RiskIndicator[];
  ...
  private async assessFraudRisk(memberId: string, member: any): Promise<{
    type: RiskType;
    level: RiskLevel;
    score: number;
    indicators: RiskIndicator[];
  ...
  private async assessBurnoutRisk(memberId: string, member: any): Promise<{
    type: RiskType;
    level: RiskLevel;
    score: number;
    indicators: RiskIndicator[];
  ...
  private async assessConflictRisk(memberId: string, member: any): Promise<{
    type: RiskType;
    level: RiskLevel;
    score: number;
    indicators: RiskIndicator[];
  ...
  private async createAlert(memberId: string, memberName: string, risk: {
    ...
    for (const listener of Array.from(this.alertListeners)) {
      try {
        ...
        console.error('[RiskPrediction] Alert listener error:', error);
      }
    }
  ...
  private async logRiskAlert(alert: RiskAlert): Promise<void> {
    try {
      ...
      console.error('[RiskPrediction] Failed to log alert:', error);
    }
  }
