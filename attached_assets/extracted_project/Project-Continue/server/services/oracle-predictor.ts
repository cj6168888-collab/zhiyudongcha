...
function parseJsonFromResponse(text: string): any {
  ...
  if (jsonMatch) {
    try {
      ...
      console.error('[Oracle] JSON解析失败:', e);
    }
  }
  return null;
}
...
请以如下JSON格式返回：
{
  ...
  try {
    ...
    console.error('[Oracle] 沟通预测失败:', error);
    throw error;
  }
}
...
请以如下JSON格式返回：
{
  ...
  try {
    ...
    console.error('[Oracle] 策略模拟失败:', error);
    throw error;
  }
}
...
请以如下JSON格式返回：
{
  ...
  try {
    ...
    console.error('[Oracle] 风险评估失败:', error);
    throw error;
  }
}
