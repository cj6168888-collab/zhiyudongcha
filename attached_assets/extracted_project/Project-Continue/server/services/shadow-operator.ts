...
class AndroidChannelAdapter {
  ...
  async connect(deviceId: string, config: { adbHost?: string; wsEndpoint?: string }): Promise<boolean> {
    console.log(`[ShadowOperator] Connecting to Android device: ${deviceId}`);
  ...
  private async executeViaAccessibility(action: ScreenAction, jitter: { x: number; y: number; delay: number }): Promise<boolean> {
    ...
    console.log(`[Android/Accessibility] Executing: ${JSON.stringify(command)}`);
  ...
  private async executeViaADB(action: ScreenAction, jitter: { x: number; y: number; delay: number }): Promise<boolean> {
    ...
    console.log(`[Android/ADB] Executing: ${adbCommand}`);
  ...
  async captureScreen(): Promise<string> {
    // 实际实现会请求屏幕截图
    console.log('[Android] Capturing screen...');
...
class PCChannelAdapter {
  ...
  async connect(config: { rpcEndpoint: string; platform: 'WINDOWS' | 'MACOS' | 'LINUX' }): Promise<boolean> {
    console.log(`[ShadowOperator] Connecting to PC: ${config.platform} at ${config.rpcEndpoint}`);
  ...
  private async sendRPCCommand(command: object): Promise<boolean> {
    ...
    console.log(`[PC/RPC] Sending command: ${JSON.stringify(command)}`);
  ...
  async captureScreen(): Promise<string> {
    console.log('[PC] Capturing screen...');
    return 'base64_screenshot_data';
  }
...
class ShadowOperatorService {
  ...
  constructor() {
    console.log('[ShadowOperator] Service initialized');
  }
  ...
  async registerDevice(
    ...
    console.log(`[ShadowOperator] Device registered: ${name} (${deviceType}) -> ${sessionId}`);
    return session;
  }
  ...
  async unregisterDevice(sessionId: string): Promise<boolean> {
    ...
    console.log(`[ShadowOperator] Device unregistered: ${sessionId}`);
    return true;
  }
  ...
  private async processCommand(command: ShadowCommand): Promise<void> {
    ...
    if (command.verification.enabled && screenshotBefore && screenshotAfter) {
      ...
      if (!result.changeDetected && result.success) {
        console.log(`[ShadowOperator] Warning: No visual change detected after action`);
      }
    }
    ...
    if (result.success) {
      ...
      if (fallbackSuggestion.shouldFallback && command.channel !== 'VLLM') {
        console.log(`[ShadowOperator] ${fallbackSuggestion.reason}`);
        console.log(`[ShadowOperator] 切换到 VLLM 视觉识别模式...`);
        ...
        try {
          if (screenshotAfter && command.action.target) {
            ...
            if (groundingResult.found && groundingResult.coordinates) {
              ...
              console.log(`[ShadowOperator] VLLM 定位成功，坐标: (${groundingResult.coordinates.x}, ${groundingResult.coordinates.y})`);
              await this.processCommand(command);
              return;
            }
          }
        } catch (vllmError) {
          console.log(`[ShadowOperator] VLLM 后备也失败: ${vllmError}`);
        }
      }
      ...
      if (command.verification.retryOnFailure && command.attempts < command.maxAttempts) {
        command.status = 'RETRY';
        console.log(`[ShadowOperator] Retrying command (${command.attempts}/${command.maxAttempts})`);
        await this.processCommand(command);
        return;
      }
      command.status = 'FAILED';
