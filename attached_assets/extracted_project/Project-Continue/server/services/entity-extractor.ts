...
export class EntityExtractor {
  ...
  async extractFromText(text: string): Promise<ExtractionResult> {
    if (!this.dashscopeApiKey) {
      console.warn('[EntityExtractor] No API key, using regex fallback');
    ...
    try {
      ...
      if (!response.ok) {
        console.error('[EntityExtractor] API error:', response.status);
        return this.regexFallback(text);
      }
      ...
      if (!jsonMatch) {
        console.error('[EntityExtractor] No JSON in response');
        return this.regexFallback(text);
      }
  ...
  async createPersonsFromExtraction(extraction: ExtractionResult, options?: {
    autoConfirm?: boolean;
    source?: string;
  }): Promise<{ created: number; updated: number; skipped: number; persons: any[] }> {
    const result = { created: 0, updated: 0, skipped: 0, persons: [] as any[] };
    ...
    for (const extracted of extraction.persons) {
      ...
      try {
        ...
        if (existing) {
          ...
          const [created] = await db.insert(unifiedContacts).values({
            name: extracted.name,
            role: extracted.role,
        ...
        console.error('[EntityExtractor] Error creating contact:', error);
        result.skipped++;
      }
    }
  ...
  async extractAndHarvest(text: string, options?: {
    ...
    harvest: { created: number; updated: number; skipped: number; persons: any[] };
  }> {
