...
export class OllamaProvider implements LLMProvider {
  ...
  async chat(messages: LLMMessage[], options?: LLMGenerateOptions): Promise<LLMResponse | null> {
    ...
    try {
      ...
      console.error('[Ollama] Request failed:', error instanceof Error ? error.message : 'Unknown');
      return null;
    }
  }
  ...
  async *chatStream(messages: LLMMessage[], options?: LLMGenerateOptions): AsyncGenerator<string, void, unknown> {
    const available = await this.isAvailable();
    if (!available) {
      console.log('[Ollama] Service not available for streaming');
      return;
    }
    ...
    try {
      ...
      console.error('[Ollama] Stream error:', error instanceof Error ? error.message : 'Unknown');
    }
  }
