...
export class DocumentDecoderService {
  ...
  async getDocument(id: string): Promise<UploadedDocument | null> {
    const [doc] = await db.select().from(uploadedDocuments).where(eq(uploadedDocuments.id, id));
    return doc || null;
  }

  async getDocuments(limit = 50): Promise<UploadedDocument[]> {
    return db.select().from(uploadedDocuments).orderBy(desc(uploadedDocuments.createdAt)).limit(limit);
  }

  async addPage(data: InsertDocumentPage): Promise<DocumentPage> {
    const [page] = await db.insert(documentPages).values(data).returning();
    return page;
  }
  ...
  private async decodeImage(filePath: string): Promise<DecodeResult> {
    try {
      ...
      return { success: false, error: `图片OCR失败: ${error.message}` };
    }
  }
  ...
  private async decodePDF(filePath: string): Promise<DecodeResult> {
    try {
      ...
      return { success: false, error: `PDF解析失败: ${error.message}` };
    }
  }
  ...
  private async decodeWord(filePath: string): Promise<DecodeResult> {
    try {
      ...
      return { success: false, error: `Word解析失败: ${error.message}` };
    }
  }
  ...
  private async decodeExcel(filePath: string): Promise<DecodeResult> {
    try {
      ...
      return { success: false, error: `Excel解析失败: ${error.message}` };
    }
  }
  ...
  private async decodeText(filePath: string): Promise<DecodeResult> {
    try {
      ...
      return { success: false, error: `文本读取失败: ${error.message}` };
    }
  }
...
返回格式：
{
  ...
  async getOrCreateThread(documentId: string): Promise<DocumentThread> {
    const [existing] = await db.select().from(documentThreads)
      .where(eq(documentThreads.documentId, documentId));
    
    if (existing) return existing;

    const doc = await this.getDocument(documentId);
    const [thread] = await db.insert(documentThreads).values({
      documentId,
  ...
  async getThread(documentId: string): Promise<DocumentThread | null> {
    const [thread] = await db.select().from(documentThreads)
      .where(eq(documentThreads.documentId, documentId));
    return thread || null;
  }

  async listMessages(documentId: string, limit = 50): Promise<DocumentMessage[]> {
    return db.select().from(documentMessages)
  ...
  async appendMessage(data: InsertDocumentMessage): Promise<DocumentMessage> {
    const [message] = await db.insert(documentMessages).values(data).returning();
    
    // 更新线程消息计数
    await db.update(documentThreads)
      .set({ 
  ...
  async applyMessageToStructuredData(messageId: string): Promise<void> {
    const [message] = await db.select().from(documentMessages)
    ...
    const currentData = (doc.structuredData as any) || {};
    const diff = message.structuredDiff as any;

    // 应用修改
    if (diff.action === "update" && diff.field) {
      currentData[diff.field] = diff.newValue;
    } else if (diff.action === "add" && diff.field) {
      if (Array.isArray(currentData[diff.field])) {
        currentData[diff.field].push(diff.newValue);
      } else {
        currentData[diff.field] = diff.newValue;
      }
    ...
    await db.update(documentMessages)
      .set({ isApplied: true })
      .where(eq(documentMessages.id, messageId));
  }
  ...
  async generateAIAnalysis(text: string, structuredData: any): Promise<string> {
    const apiKey = process.env.DASHSCOPE_API_KEY;
    if (!apiKey) {
...
${text.slice(0, 2000)}`,
  ...
  private generateFallbackAnalysis(text: string, structuredData: any): string {
    const analysis: string[] = ["📋 文档分析结果："];
    
    if (structuredData?.parties?.length > 0) {
      analysis.push(`\n**合同方**: ${structuredData.parties.map((p: any) => p.name).join("、")}`);
    }
    if (structuredData?.amounts?.length > 0) {
      analysis.push(`\n**涉及金额**: ${structuredData.amounts.map((a: any) => `${a.currency} ${a.value}`).join("、")}`);
    }
    if (structuredData?.dates?.length > 0) {
      analysis.push(`\n**重要日期**: ${structuredData.dates.map((d: any) => d.date).join("、")}`);
    }
    if (structuredData?.risks?.length > 0) {
      analysis.push(`\n**风险提示**: ${structuredData.risks.map((r: any) => r.description).join("；")}`);
    }
  ...
  async addSupplement(data: InsertDocumentSupplement): Promise<DocumentSupplement> {
    const [supplement] = await db.insert(documentSupplements).values(data).returning();
    return supplement;
  }

  async getSupplements(documentId: string): Promise<DocumentSupplement[]> {
    return db.select().from(documentSupplements)
  ...
  async processSupplement(supplementId: string): Promise<void> {
    const [supplement] = await db.select().from(documentSupplements)
      .where(eq(documentSupplements.id, supplementId));
    
    if (!supplement) return;

    await db.update(documentSupplements)
...
请用简洁专业的语言，分点说明。称呼主人为"爸爸"。`,
      ...
      if (!response.ok) {
        return this.generateFallbackAnalysis(fullContext, doc?.structuredData);
      }

      const data: any = await response.json();
      return data.choices?.[0]?.message?.content || this.generateFallbackAnalysis(fullContext, doc?.structuredData);
    } catch (error) {
      return this.generateFallbackAnalysis(fullContext, doc?.structuredData);
    }
  }
