...
class VisionEvolverService {
  ...
  async evolve(): Promise<VisionEvolutionResult> {
    console.log('[VisionEvolver] 开始视觉进化...');
    
    const samples = await this.collectFailedRecognitions();
    console.log(`[VisionEvolver] 发现${samples.length}个需要重新学习的样本`);
  ...
  private async learnPatternFromSamples(
    ...
    console.log(`[VisionEvolver] 从${samples.length}个样本学习${type}模式...`);
  ...
  async teachPattern(
    ...
    const [pattern] = await db.insert(visionPatterns).values({
      patternType,
      patternName,
      description: `用户教授的模式`,
      recognitionRules: rules as any,
      trainingCount: 1,
      accuracy: 0.9,
      isActive: 1,
    }).returning();
