...
type StorageWithExtras = {
  ...
  createRagKnowledge?: (data: any) => Promise<RagKnowledge>;
  getCalendarEvents?: (options: any) => Promise<any[]>;
...
export async function generateEmbedding(text: string): Promise<number[] | null> {
  if (!DASHSCOPE_API_KEY) {
    console.warn('[Embedding] API key not configured');
    return null;
  }
  ...
  try {
    ...
    if (embedding && Array.isArray(embedding)) {
      console.log(`[Embedding] Generated vector of ${embedding.length} dimensions`);
      return embedding;
    }
...
export async function batchGenerateEmbeddings(texts: string[]): Promise<(number[] | null)[]> {
  ...
  try {
    ...
    console.error('[BatchEmbedding] Error:', error);
    return texts.map(() => null);
  }
}
...
export async function semanticSearch(
  ...
  try {
    ...
    console.log(`[SemanticSearch] Found ${scored.length} results for "${query.slice(0, 30)}..."`);
    return scored;
  } catch (error) {
...
export async function executeEnhancedToolCall(
  ...
  console.log(`[EnhancedTool] Executing: ${name}`, args);

  switch (name) {
    case 'search_knowledge': {
      if (!storage.getAllRagKnowledge) {
        console.log('[EnhancedTool] search_knowledge: storage.getAllRagKnowledge not available');
        return '知识库功能暂未启用。';
      }
      const results = await semanticSearch(args.query, storage, {
    ...
    case 'query_calendar': {
      if (!storage.getCalendarEvents) {
        console.log('[EnhancedTool] query_calendar: storage.getCalendarEvents not available');
        return '日程功能暂未启用。';
      }
      const events = await storage.getCalendarEvents({
        startDate: args.date === 'today' ? new Date() : 
                   args.date === 'tomorrow' ? new Date(Date.now() + 86400000) :
                   args.date ? new Date(args.date) : new Date(),
        daysAhead: args.daysAhead || 1,
      });
    ...
    case 'check_health_status': {
      if (!storage.getHPState) {
        console.log('[EnhancedTool] check_health_status: storage.getHPState not available');
        return 'HP状态功能暂未启用。';
      }
    ...
    case 'web_search': {
      const searchResult = await webSearchWithQwen(args.query, args.time_range);
      return searchResult;
    }

    case 'generate_image': {
      const imageResult = await generateImageWithWanxiang(args.prompt, {
        style: args.style,
        size: args.size,
      });
      return imageResult;
    ...
    case 'execute_code': {
      const codeResult = await executeCodeSandbox(args.code, args.purpose);
      return codeResult;
    }

    default:
      return await executeToolCall(toolCall, storage);
  }
}
...
export async function analyzeImage(
  ...
  try {
    ...
    if (!response.ok) {
      console.error('[ImageAnalysis] API error:', response.status);
      return '图像分析暂时不可用';
    }
...
export async function transcribeAudioParaformer(
  ...
  try {
    ...
    if (!submitResponse.ok) {
      console.error('[Paraformer] Submit error:', submitResponse.status);
    ...
    console.error('[Paraformer] Error:', error);
    return { text: '语音识别出错' };
  }
}
...
export async function ingestKnowledge(
  ...
  try {
    ...
    if (result) {
      console.log(`[IngestKnowledge] Created knowledge: ${result.id}`);
...
export async function webSearchWithQwen(
  ...
  try {
    ...
    const data = await response.json();
    console.log('[WebSearch] Raw response:', JSON.stringify(data).slice(0, 500));
    ...
    if (searchInfo) {
      console.log('[WebSearch] Search info available:', !!searchInfo);
    }
    
    console.log(`[WebSearch] Query: "${query.slice(0, 30)}..." completed`);
    return content;
  } catch (error) {
...
export async function generateImageWithWanxiang(
  ...
  try {
    ...
    if (!submitResponse.ok) {
      ...
      console.error('[ImageGen] Submit error:', submitResponse.status, errorText);
      return '图片生成提交失败';
    }
    ...
    if (!taskId) {
      console.error('[ImageGen] No task_id in response:', submitData);
      return '无法获取图片生成任务ID';
    }
    ...
    while (attempts < maxAttempts) {
      ...
      if (status === 'SUCCEEDED') {
        ...
        console.error('[ImageGen] No image URL in results:', results);
...
export async function executeCodeSandbox(
  ...
  console.log(`[CodeExec] Purpose: ${purpose || 'unspecified'}`);
...
注意：你只需要模拟执行并给出结果，不需要真正运行代码。`
    ...
    if (!response.ok) {
      console.error('[CodeExec] API error:', response.status);
      return '代码分析暂时不可用';
    }
    ...
    console.log('[CodeExec] Code analysis completed');
    return content;
  } catch (error) {
...
console.log('[DashScope Enhanced] Module loaded with Embedding, RAG, Multimodal, ASR, WebSearch, ImageGen, and CodeExec capabilities');
