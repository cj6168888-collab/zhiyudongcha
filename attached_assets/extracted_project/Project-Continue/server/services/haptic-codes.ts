...
class HapticCodesService {
  ...
  private activeConnections: Map<string, { deviceType: string; ws?: any }> = new Map();
  ...
  async sendHapticSignal(data: {
    ...
    console.log(`[HapticCodes] 发送暗号: ${data.code} - ${data.context}`);
    ...
    for (const device of message.deviceTargets) {
      ...
      if (connection) {
        try {
          ...
          console.error(`[HapticCodes] 发送到 ${device} 失败:`, error);
        }
      }
    }
  }
  
  private findDeviceConnection(deviceType: string): { deviceType: string; ws?: any } | undefined {
  ...
  registerDevice(deviceId: string, deviceType: 'PHONE' | 'GLASSES' | 'WATCH', ws?: any): void {
    this.activeConnections.set(deviceId, { deviceType, ws });
    console.log(`[HapticCodes] 设备已注册: ${deviceId} (${deviceType})`);
  }
  
  unregisterDevice(deviceId: string): void {
    this.activeConnections.delete(deviceId);
    console.log(`[HapticCodes] 设备已注销: ${deviceId}`);
  }
