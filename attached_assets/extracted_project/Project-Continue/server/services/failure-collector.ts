...
class FailureCollectorService {
  ...
  async collectUnanswered(query: string, reason: string, context: FailureContext): Promise<string> {
    console.log(`[FailureCollector] Collecting UNANSWERED: "${query.slice(0, 50)}..."`);
    
    const encryptedQuery = this.encrypt(query);
    const encryptedReason = this.encrypt(reason);
    
    const [failure] = await db.insert(failureTags).values({
      tagType: 'UNANSWERED',
  ...
  async collectExecutionError(
    ...
    console.log(`[FailureCollector] Collecting EXECUTION_ERROR: ${operation}`);
  ...
  async collectUserCorrection(
    ...
    console.log(`[FailureCollector] Collecting USER_CORRECTION`);
  ...
  async collectLowConfidence(
    ...
    console.log(`[FailureCollector] Collecting LOW_CONFIDENCE (${confidence})`);
    ...
    const [failure] = await db.insert(failureTags).values({
      ...
      context: { ...context, confidence } as any,
      deviceId: context.deviceId,
      isProcessed: 0,
    }).returning();
  ...
  async collectTimeout(
    ...
    console.log(`[FailureCollector] Collecting TIMEOUT (${timeoutMs}ms)`);
    ...
    const [failure] = await db.insert(failureTags).values({
      ...
      context: { ...context, timeoutMs } as any,
      deviceId: context.deviceId,
      isProcessed: 0,
    }).returning();
  ...
  async scanChatHistoryForFailures(): Promise<number> {
    ...
    const negativeFeedback = await db.select()
    ...
    for (const chat of negativeFeedback) {
      const prevUserChat = await db.select()
        .from(avatarChatHistory)
        .where(and(
  ...
  async scanAuditLogsForFailures(): Promise<number> {
    ...
    const failedAudits = await db.select()
      .from(auditLogs)
      .where(and(
  ...
  async markAsProcessed(failureId: string, evolutionResult: Record<string, any>): Promise<void> {
    await db.update(failureTags)
      .set({
        isProcessed: 1,
        processedAt: new Date(),
        evolutionResult: evolutionResult as any,
      })
