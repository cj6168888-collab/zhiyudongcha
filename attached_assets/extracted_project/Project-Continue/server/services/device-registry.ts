...
export class DeviceRegistry extends EventEmitter {
  ...
  registerDevice(
    ...
    console.log(`[DeviceRegistry] Device registered: ${name} (${deviceId}) - ${type}`);
    console.log(`[DeviceRegistry] Capabilities: LocalAI=${capabilities.canRunLocalModel}, GPU=${capabilities.gpuAvailable}`);
  ...
  unregisterDevice(deviceId: string): boolean {
    ...
    console.log(`[DeviceRegistry] Device unregistered: ${device.name} (${deviceId})`);
  ...
  sendToDevice(deviceId: string, message: any): boolean {
    ...
    try {
      ...
      console.error(`[DeviceRegistry] Failed to send to device ${deviceId}:`, error);
      return false;
    }
  }
    ...
    this.checkInterval = setInterval(() => {
      ...
      for (const [id, device] of entries) {
        if (device.status !== 'OFFLINE' && now - device.lastHeartbeat > this.offlineThreshold) {
          device.status = 'OFFLINE';
          console.log(`[DeviceRegistry] Device offline (no heartbeat): ${device.name} (${id})`);
