...
class EmotionalMemoryService {
  ...
  async generateEmbedding(text: string): Promise<number[]> {
    ...
    if (!DASHSCOPE_API_KEY) {
      console.warn('[EmotionalMemory] No DashScope API key, using simple hash embedding');
    ...
    try {
      ...
      console.warn('[EmotionalMemory] Embedding API failed, using fallback');
      return this.simpleHashEmbedding(text);
    } catch (error) {
      console.error('[EmotionalMemory] Embedding error:', error);
      return this.simpleHashEmbedding(text);
    }
  }
  ...
  async extractEntities(conversation: string, userRole: 'MASTER' | 'GUEST' = 'MASTER'): Promise<ExtractedEntity[]> {
    if (!DASHSCOPE_API_KEY) {
      console.warn('[EmotionalMemory] No API key, using pattern-based extraction');
    ...
    try {
      ...
      if (jsonMatch) {
        ...
        console.log(`[EmotionalMemory] Extracted ${entities.length} entities from conversation`);
        return entities;
      }

      return [];
    } catch (error) {
      console.error('[EmotionalMemory] Entity extraction error:', error);
      return this.patternBasedExtraction(conversation);
    }
  }
  ...
  async storeMemory(entity: ExtractedEntity): Promise<EmotionalMemory> {
    ...
    if (existing.length > 0) {
      const [updated] = await db.update(emotionalMemories)
        .set({
      ...
      console.log(`[EmotionalMemory] Updated memory: ${entity.type}/${entity.key}`);
      return updated;
    }

    const [created] = await db.insert(emotionalMemories)
      .values({
    ...
    console.log(`[EmotionalMemory] Created memory: ${entity.type}/${entity.key}`);
    return created;
  }
  ...
  async processConversation(conversationText: string, conversationId?: string, userRole: 'MASTER' | 'GUEST' = 'MASTER'): Promise<{
    ...
    for (const entity of entities) {
      try {
        ...
        console.error(`[EmotionalMemory] Failed to store entity:`, error);
      }
    }

    if (conversationId && entities.length > 0) {
      await db.insert(conversationEmotionalContext).values({
        conversationId,
  ...
  async getProactiveCareInstructions(): Promise<ProactiveCareInstruction[]> {
    ...
    const healthMemories = await db.select()
      .from(emotionalMemories)
      .where(and(
    ...
    for (const memory of healthMemories) {
      if (!memory.followUpTriggeredAt || 
        ...
        await db.update(emotionalMemories)
          .set({ followUpTriggeredAt: now })
          .where(eq(emotionalMemories.id, memory.id));
      }
    }

    const moodMemories = await db.select()
      .from(emotionalMemories)
      .where(and(
  ...
  async getMemoryStats(): Promise<{
    ...
    const allMemories = await db.select().from(emotionalMemories);
...
console.log('[EmotionalMemory] 情感记忆服务已初始化 - "记得爸爸一切"');
