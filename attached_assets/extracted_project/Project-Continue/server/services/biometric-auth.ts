...
class BiometricAuthService {
  constructor() {
    console.log('[BiometricAuth] 生物特征认证服务已初始化 (Phase 1.4 持久化版)');
    console.log('[BiometricAuth] 支持: 面容识别(Qwen-VL双图比对)、指纹识别、声纹识别');
    console.log('[BiometricAuth] 数据库持久化: 已启用');
    console.log('[BiometricAuth] 验证失败锁定: 5次失败锁定30分钟');
  }

  async getProfileFromDB(userId: string): Promise<DBBiometricProfile | null> {
    try {
      const result = await db.select().from(biometricProfiles).where(eq(biometricProfiles.userId, userId)).limit(1);
      return result[0] || null;
    } catch (error) {
      console.error('[BiometricAuth] DB read error:', error);
      return null;
    }
  }
  ...
  async createProfileInDB(userId: string): Promise<DBBiometricProfile | null> {
    try {
      ...
      console.error('[BiometricAuth] DB create error:', error);
      return null;
    }
  }
  ...
  async isAccountLocked(userId: string): Promise<{ locked: boolean; remainingMs?: number }> {
    ...
    await db.update(biometricProfiles)
  ...
  async recordVerificationAttempt(
    ...
    try {
      await db.insert(biometricVerificationLogs).values({
        userId,
      ...
      if (success) {
        ...
        if (newFailedCount >= LOCK_THRESHOLD) {
          ...
          console.log('[BiometricAuth] 账户已锁定:', userId, '锁定至:', updates.lockedUntil);
        }
      ...
      console.error('[BiometricAuth] Record verification error:', error);
    }
  }
  ...
  async enrollFace(userId: string, imageBase64: string, deviceId?: string): Promise<BiometricEnrollResult> {
    try {
      ...
      console.log('[BiometricAuth] 面容录入成功 (持久化):', userId, '质量:', qualityResult.quality);
      ...
      console.error('[BiometricAuth] Face enrollment error:', error);
      return {
        success: false,
        type: 'FACE',
        message: '面容录入失败'
  ...
  async verifyFace(
    ...
    try {
      ...
      console.log('[BiometricAuth] 面容验证 (持久化):', userId, {
      ...
      console.error('[BiometricAuth] Face verify error:', error);
      await this.recordVerificationAttempt(userId, 'FACE', false, 0, {
        deviceId,
  ...
  async verifyFaceSimple(userId: string, imageBase64: string): Promise<BiometricVerifyResult> {
    ...
    try {
      ...
      console.error('[BiometricAuth] Face simple verify error:', error);
      return {
        success: false,
        verified: false,
        type: 'FACE',
  ...
  async enrollFingerprint(
    ...
    try {
      ...
      console.log('[BiometricAuth] 指纹录入成功:', userId);
      ...
      console.error('[BiometricAuth] Fingerprint enrollment error:', error);
      return {
        success: false,
        type: 'FINGERPRINT',
        message: '指纹录入失败'
  ...
  async enrollVoice(userId: string, sampleCount: number, confidenceThreshold: number): Promise<BiometricEnrollResult> {
    try {
      ...
      console.log('[BiometricAuth] 声纹状态更新:', userId, sampleCount);
      ...
      console.error('[BiometricAuth] Voice enrollment error:', error);
      return {
        success: false,
        type: 'VOICE',
        message: '声纹录入失败'
  ...
  async unlockAccount(userId: string): Promise<void> {
    ...
    console.log(`[BiometricAuth] 账户已手动解锁: ${userId}`);
  }
}
