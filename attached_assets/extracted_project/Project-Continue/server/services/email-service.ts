...
export class EmailService {
  ...
  async syncAccount(accountId: string, limit: number = 50): Promise<SyncResult> {
    ...
    try {
      ...
      console.log(`[Email Sync] Starting sync for ${account.email}`);
      ...
      await client.connect();
      console.log(`[Email Sync] Connected to ${account.imapHost}`);
      ...
      try {
        ...
        for await (const message of client.fetch(`1:${limit}`, { 
          ...
          try {
            ...
            console.log(`[Email Sync] Synced: ${subject.substring(0, 30)}...`);
          } catch (parseError) {
            console.error('[Email Sync] Parse error:', parseError);
        ...
        result.success = true;
        console.log(`[Email Sync] Completed. New emails: ${result.newEmails}`);
      } finally {
        lock.release();
      }
