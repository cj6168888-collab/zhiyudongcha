...
class SkillLearnerService {
  ...
  async teachSkill(input: TeachSkillInput): Promise<LearnedSkill> {
    const [skill] = await db.insert(learnedSkills).values({
  ...
  async matchSkill(userInput: string): Promise<SkillMatchResult | null> {
    const activeSkills = await db.select().from(learnedSkills)
  ...
  async executeSkill(skillId: string): Promise<{ success: boolean; output: string; steps: SkillStep[] }> {
    const [skill] = await db.select().from(learnedSkills)
  ...
  async identifyPattern(userMessage: string): Promise<PatternMatchResult | null> {
    const patterns = await db.select().from(conversationPatterns)
  ...
  async recordPattern(
    ...
    const existing = await db.select().from(conversationPatterns)
      .where(ilike(conversationPatterns.patternText, `%${patternText.substring(0, 50)}%`))
      .limit(1);
    
    if (existing.length > 0) {
      await db.update(conversationPatterns)
        .set({
    ...
    const [pattern] = await db.insert(conversationPatterns).values({
      patternType,
      patternText,
      keywords,
      responseTemplate,
  ...
  async listSkills(category?: string): Promise<LearnedSkill[]> {
    if (category) {
      return db.select().from(learnedSkills)
        .where(and(
  ...
  async listPatterns(patternType?: string): Promise<ConversationPattern[]> {
    if (patternType) {
      return db.select().from(conversationPatterns)
  ...
  async updateSkill(skillId: string, updates: Partial<InsertLearnedSkill>): Promise<LearnedSkill | null> {
    const [updated] = await db.update(learnedSkills)
      .set({
        ...updates,
        updatedAt: new Date(),
      })
  ...
  async getSkillStats(): Promise<{
    ...
    const allSkills = await db.select().from(learnedSkills);
    const activeSkills = allSkills.filter(s => s.isActive === 1);
    
    const allPatterns = await db.select().from(conversationPatterns);
    const autoRespondPatterns = allPatterns.filter(p => p.autoRespond === 1);
    
    const topSkills = await db.select().from(learnedSkills)
      .where(eq(learnedSkills.isActive, 1))
      .orderBy(desc(learnedSkills.usageCount))
      .limit(5);
    
    const topPatterns = await db.select().from(conversationPatterns)
      .orderBy(desc(conversationPatterns.occurrenceCount))
      .limit(5);
  ...
  async recordFeedback(input: RecordFeedbackInput): Promise<SkillFeedback> {
    const [feedback] = await db.insert(skillFeedback).values({
  ...
  async getFeedbackHistory(options?: {
    ...
    let query = db.select().from(skillFeedback);
  ...
  async recordFollowup(conversationId: string, messageId: string): Promise<void> {
    const existing = await db.select().from(skillFeedback)
      .where(and(
  ...
  private async updateProficiencyCurve(category: string, feedbackType: FeedbackType): Promise<void> {
    ...
    const existing = await db.select().from(skillProficiencyCurve)
      .where(and(
    ...
    if (existing.length > 0) {
      ...
      await db.update(skillProficiencyCurve)
        .set(updates)
      ...
      await db.insert(skillProficiencyCurve).values({
        category,
        date: today,
        totalInteractions: 1,
  ...
  private async recalculateProficiency(recordId: string): Promise<void> {
    const [record] = await db.select().from(skillProficiencyCurve)
  ...
  async getProficiencyCurve(options?: {
    ...
    let query = db.select().from(skillProficiencyCurve);
  ...
  async getCategorySummary(): Promise<{
    ...
    const allCategories = await db.select().from(skillProficiencyCurve)
      .orderBy(desc(skillProficiencyCurve.date));
  ...
  async analyzeFollowupPatterns(): Promise<{
    ...
    const followups = await db.select().from(skillFeedback)
