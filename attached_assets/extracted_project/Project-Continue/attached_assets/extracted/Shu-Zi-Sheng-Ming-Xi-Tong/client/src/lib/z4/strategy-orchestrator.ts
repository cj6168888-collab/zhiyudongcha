...
import { z2CoreApi, type RelationshipInsight } from '../api';
...
const createSecretaryAgent = (): SecretaryAgent => ({
  simulateGuiOp: async (targetApp, action) => {
    console.log(`[Secretary] Simulating: ${action} on ${targetApp}`);
  ...
  scheduleTask: (task, time) => {
    console.log(`[Secretary] Scheduled: ${task} at ${time.toISOString()}`);
  },
      ...
      return {
        ...
        executeSubTask: async (task) => {
          ...
          switch (task.assignedExpert) {
            case 'LEGAL':
              result = await legalExpert.auditDocument(task.description);
              break;
            case 'FINANCE':
              result = await financeExpert.calcRoi({});
              break;
            case 'STRATEGY':
        ...
        runStrategyPlanning: async (context, opponentName) => {
          ...
          if (opponentName) {
            try {
              personContext = await z2CoreApi.getRelationshipInsight(opponentName);
            } catch (e) {
              console.log('[Z4] Could not fetch opponent insight');
            }
          }
        ...
        delayAggressiveAction: (actionId) => {
          ...
          console.log(`[Psychology] Action ${actionId} delayed due to emotional state`);
        },

        // Output Dispatch
        dispatchOutput: (content) => {
          const z3 = useZ3Store.getState();
          console.log(`[Z4] Dispatching results via ${content.outputMode}`);
