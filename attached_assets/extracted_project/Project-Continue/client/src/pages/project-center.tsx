...
type ProjectStatus = 'PENDING' | 'PENDING_REVIEW' | 'APPROVED' | 'ON_HOLD' | 'DEPRECATED' | 'COMPLETED';
...
const defaultStatusConfig = { label: '未知', color: 'bg-gray-500/20 text-gray-400 border-gray-500/30', icon: <Clock className="w-3 h-3" /> };

const statusConfig: Record<string, { label: string; color: string; icon: React.ReactNode }> = {
  PENDING: { label: '待定', color: 'bg-yellow-500/20 text-yellow-400 border-yellow-500/30', icon: <Clock className="w-3 h-3" /> },
  PENDING_REVIEW: { label: '待审', color: 'bg-purple-500/20 text-purple-400 border-purple-500/30', icon: <Clock className="w-3 h-3" /> },
...
export default function ProjectCenter() {
  ...
  const { data: allFiles = [] } = useQuery<GeneratedFile[]>({
    queryKey: ['/api/generated-files'],
    refetchInterval: 30000,
  });

  const getProjectFiles = (projectId: string) => 
    allFiles.filter(f => f.relatedProjectId === projectId);

  const getFileTypeLabel = (type: string) => {
    const labels: Record<string, string> = { CONTRACT: '合同', PROPOSAL: '策划书', REPORT: '报告', ANALYSIS: '分析', MEMO: '备忘录' };
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: "项目已创建", description: "新项目已添加到项目列表" });
    },
  });
  ...
  const filteredProjects = projects.filter((p: Project) => {
    ...
    if (filterStatus === 'PENDING_REVIEW') return p.status === 'PENDING' || p.status === 'PENDING_REVIEW';
    return p.status === filterStatus;
  });
  const pendingCount = projects.filter((p: Project) => p.status === 'PENDING' || p.status === 'PENDING_REVIEW').length;
  ...
  return (
    <div className="min-h-screen bg-background px-6 pt-6 pb-24 md:pb-6" data-testid="project-center-page">
      <div className="max-w-7xl mx-auto space-y-6">
        <GlobalWakeHeader 
          title="项目管理中心" 
          subtitle="Project Management Center"
          rightActions={
            <div className="flex flex-wrap items-center gap-2 md:gap-3">
              <Link href="/projects/dashboard">
                <Button variant="outline" size="sm" className="border-blue-500/30 text-blue-400 hover:bg-blue-500/10" data-testid="button-dashboard">
                  <BarChart3 className="w-4 h-4 mr-2" />
                  仪表盘
                </Button>
              </Link>
              <Link href="/projects/smart-create">
                <Button variant="outline" size="sm" className="border-purple-500/30 text-purple-400 hover:bg-purple-500/10" data-testid="button-smart-create">
                  <Brain className="w-4 h-4 mr-2" />
                  智能创建
                </Button>
              </Link>
              <Link href="/projects/templates">
                <Button variant="outline" size="sm" className="border-amber-500/30 text-amber-400 hover:bg-amber-500/10" data-testid="button-templates">
                  <BookTemplate className="w-4 h-4 mr-2" />
                  模板库
                </Button>
              </Link>
              <Badge variant="outline" className="bg-purple-500/20 text-purple-400" data-testid="badge-pending">{pendingCount} 待审</Badge>
        ...
        <Tabs value={filterStatus} onValueChange={setFilterStatus}>
          <TabsList>
            <TabsTrigger value="PENDING_REVIEW" data-testid="tab-pending">待审</TabsTrigger>
                  ...
                  <ScrollArea className="h-[500px]">
                    <div className="space-y-3">
                      {filteredProjects.map((project: Project) => (
                        ...
                        <div 
                          className="p-4 rounded-lg border cursor-pointer transition-colors hover:bg-muted/50 border-border"
                        ...
                        >
                          <div className="flex items-start justify-between gap-3 mb-3">
                            ...
                            <Badge variant="outline" className={getStatusConfig(project.status).color} data-testid={`badge-status-${project.id}`}>
