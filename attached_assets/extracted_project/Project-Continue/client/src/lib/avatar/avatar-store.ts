...
interface AvatarState {
  ...
  speakingEndTime: number; // 记录TTS结束时间，用于冷却期过滤
...
export const useAvatarStore = create<AvatarState>()(
  persist(
    (set, get) => ({
      ...
      speak: async (text: string) => {
        ...
        try {
          ...
          if (saved) {
            ...
            voiceSettings.voiceType = parsed.voiceType || 'longanhuan';
            voiceSettings.voiceSpeed = parsed.voiceSpeed || 1.0;
            voiceSettings.voicePitch = parsed.voicePitch || 1.0;
          }
        } catch {}
      ...
      setCurrentTtsText: (text: string) => set({ currentTtsText: text }),
    }),
    {
