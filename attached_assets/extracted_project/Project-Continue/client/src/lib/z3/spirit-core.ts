...
// Device Types
export type DeviceType = 'MASTER_PC' | 'MOBILE_TACTICAL' | 'AR_GLASSES' | 'INMO_GO3' | 'IOT_HUB' | 'TV';
...
interface Z3State {
  ...
  // Actions - Visual & Haptic
  setVisualOverlay: (enabled: boolean) => void;
...
export const useZ3Store = create<Z3State>()(
  persist(
    (set, get) => ({
      ...
      // Visual & Haptic Controls
      setVisualOverlay: (enabled) => {
        set((state) => ({
          streamConfig: { ...state.streamConfig, visualOverlay: enabled }
        }));
        console.log(`[SpiritCore] Visual overlay ${enabled ? '启用' : '禁用'}`);
      },

      setHapticEnabled: (enabled) => {
        set((state) => ({
          streamConfig: { ...state.streamConfig, hapticEnabled: enabled }
        }));
        // Trigger haptic feedback if enabling
        if (enabled && navigator.vibrate) {
          navigator.vibrate(50);
        }
        console.log(`[SpiritCore] Haptic feedback ${enabled ? '启用' : '禁用'}`);
      },
...
export async function initZ3WebSocket(url?: string) {
  const getAuthWsUrl = async () => {
    const protocol = window.location.protocol === 'https:' ? 'wss:' : 'ws:';
    const baseUrl = `${protocol}//${window.location.host}/ws/z3`;
    ...
    try {
      ...
      console.error('[Z3] Failed to get WS token:', e);
    }
    return baseUrl;
  };
  const wsUrl = url || await getAuthWsUrl();
