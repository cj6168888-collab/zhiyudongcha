...
export class LocalWhisperSTT {
  ...
  private async checkLocalAvailability(): Promise<void> {
    ...
    try {
      ...
      console.log(`[WhisperLocal] Local service: ${this.localAvailable ? 'ONLINE' : 'OFFLINE'}`);
    } catch {
      this.localAvailable = false;
      console.log('[WhisperLocal] Local service: OFFLINE (connection failed)');
    }
  }
  ...
  private async transcribeLocal(audioBlob: Blob): Promise<TranscriptionResult | null> {
    ...
    try {
      ...
      console.log(`[WhisperLocal] Transcribed in ${latency}ms: "${data.text}"`);
  ...
  async start(): Promise<void> {
    ...
    try {
      ...
      console.log(`[WhisperLocal] Started in ${this.config.mode} mode`);
  ...
  setMode(mode: STTMode): void {
    ...
    console.log(`[WhisperLocal] Mode changed to: ${mode}`);
  }
}
