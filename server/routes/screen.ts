...
const logger = createServiceLogger('Screen');
...
export const registerScreenRoutes: RegisterRouteFn = (
  ...
  app.post("/api/intent/map", async (req, res) => {
    try {
      ...
      logger.error({ err: error }, 'Intent mapping error');
  ...
  app.post("/api/intent/batch", async (req, res) => {
    try {
      ...
      logger.error({ err: error }, 'Batch intent mapping error');
  ...
  app.post("/api/intent/sequence", async (req, res) => {
    try {
      ...
      logger.error({ err: error }, 'Automation sequence error');
  ...
  app.post("/api/avatar/chat-with-action", async (req, res) => {
    try {
      ...
      logger.error({ err: error }, 'Chat with action error');
  ...
  app.post("/api/screen/update", async (req, res) => {
    try {
      ...
      logger.error({ err: error }, 'Screen content update error');
  ...
  app.post("/api/screen/analyze/:sessionId", async (req, res) => {
    try {
      ...
      logger.error({ err: error, sessionId: req.params.sessionId }, 'Screen analyze error');
  ...
  app.post("/api/screen/deep-analyze/:sessionId", async (req, res) => {
    try {
      ...
      logger.error({ err: error, sessionId: req.params.sessionId }, 'Deep analyze error');
  ...
  addAlertCallback((sessionId, alert) => {
    ...
    logger.info(`[Z3 Whisper] Broadcast alert to ${z3Clients.size} clients`);
  });
  ...
  app.post("/api/vision/start/:sessionId", requireMaster, async (req, res) => {
    try {
      ...
      logger.error({ err: error, sessionId: req.params.sessionId }, 'Vision sniffing start error');
  ...
  app.post("/api/vision/stop/:sessionId", async (req, res) => {
    try {
      ...
      logger.error({ err: error, sessionId: req.params.sessionId }, 'Vision sniffing stop error');
  ...
  app.post("/api/vision/alert/:sessionId", async (req, res) => {
    try {
      ...
      logger.error({ err: error, sessionId: req.params.sessionId }, 'Vision sniffing alert check error');
