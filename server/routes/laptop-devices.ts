...
const logger = createServiceLogger('LaptopDevices');
...
laptopDevicesRouter.post('/register', async (req, res) => {
  try {
    ...
    logger.info(`[LaptopDevices] Registered: ${config.deviceName} (${config.deviceId})`);
...
laptopDevicesRouter.delete('/devices', (req, res) => {
  try {
    ...
    logger.info(`[LaptopDevices] Removed: ${device.deviceName} (${deviceId})`);
...
logger.info('[LaptopDevices] Router initialized');
