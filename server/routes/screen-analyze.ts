...
const logger = createServiceLogger('ScreenAnalyze');
  ...
  "contentType": "文档类型"
    ...
    if (content) {
      const contentText = Array.isArray(content) 
        ? content.find((c: { text?: string }) => c.text)?.text || ''
        : content;
    ...
    logger.error({ err: error }, 'Screen analysis error');
    return {
      keywords: ['分析失败'],
      summary: getErrorMessage(error) || '分析过程出错',
      riskLevel: 0,
      emotionState: 'calm',
    };
  }
...
export function registerScreenAnalyzeRoutes(app: Express): void {
  ...
  logger.info("[ScreenAnalyze] Routes registered at /api/screen/*");
}
