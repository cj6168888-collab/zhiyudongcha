...
export function registerSkillLearnerRoutes(app: Express): void {
  ...
  app.post("/api/skills/teach", async (req, res) => {
    try {
      ...
      logger.error({ error, skillName: req.body.skillName }, 'Teach skill failed');
  ...
  app.post("/api/skills/match", async (req, res) => {
    try {
      ...
      logger.error({ error, userInput: req.body.userInput }, 'Match skill failed');
      res.status(500).json({ error: getErrorMessage(error) });
    }
  });
  ...
  app.post("/api/skills/:skillId/execute", async (req, res) => {
    try {
      ...
      logger.error({ error, skillId: req.params.skillId }, 'Execute skill failed');
      res.status(500).json({ error: getErrorMessage(error) });
    }
  });
  ...
  app.post("/api/skills/feedback", async (req, res) => {
    try {
      ...
      logger.error({ error, feedbackType: req.body.feedbackType }, 'Record feedback failed');
      res.status(500).json({ error: getErrorMessage(error) });
    }
  });
  ...
  logger.info("[SkillLearner] Routes registered at /api/skills/*, /api/patterns/*, /api/skills/proficiency/*");
}
