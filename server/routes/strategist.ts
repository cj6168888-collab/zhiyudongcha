...
const logger = createServiceLogger('Strategist');
...
export function registerStrategistRoutes(app: Express): void {
  ...
  app.get("/api/strategist/dashboard", requireMaster, async (_req, res) => {
    try {
      ...
      logger.error({ err: error }, '仪表盘获取错误');
