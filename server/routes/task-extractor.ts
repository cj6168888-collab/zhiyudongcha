...
const logger = createServiceLogger('TaskExtractor');
...
export async function registerTaskExtractorRoutes(app: Express, storage: IStorage): Promise<void> {
  ...
  app.post("/api/task-extractor/from-email/:emailId", requireMaster, async (req: Request, res: Response) => {
    try {
      ...
      logger.error({ error, emailId: req.params.emailId }, 'Email extraction failed');
      return res.status(500).json({ error: error instanceof Error ? error.message : "提取任务失败" });
    }
  });
  ...
  app.post("/api/task-extractor/from-text", requireMaster, async (req: Request, res: Response) => {
    try {
      ...
      logger.error({ error, sourceType: req.body.sourceType, sourceId: req.body.sourceId }, 'Text extraction failed');
      return res.status(500).json({ error: error instanceof Error ? error.message : "提取任务失败" });
    }
  });
  ...
  app.post("/api/task-extractor/batch-from-emails", requireMaster, async (req: Request, res: Response) => {
    try {
      ...
      logger.error({ error, emailCount: req.body.emailIds?.length }, 'Batch extraction failed');
      return res.status(500).json({ error: error instanceof Error ? error.message : "批量提取失败" });
    }
  });
  ...
  app.post("/api/task-extractor/create-calendar-events", requireMaster, async (req: Request, res: Response) => {
    try {
      ...
      logger.error({ error, taskCount: req.body.tasks?.length }, 'Create calendar events failed');
      return res.status(500).json({ error: error instanceof Error ? error.message : "创建日历事件失败" });
    }
  });
}
