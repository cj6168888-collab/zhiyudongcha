...
router.get('/events/:id', async (req, res) => {
  try {
    ...
    logger.error({ err: error, eventId: req.params.id }, '获取事件失败');
    res.status(500).json({ success: false, error: '获取事件失败' });
  }
});
...
router.put('/events/:id', async (req, res) => {
  try {
    ...
    logger.error({ err: error, eventId: req.params.id }, '更新事件失败');
...
router.delete('/events/:id', async (req, res) => {
  try {
    ...
    logger.error({ err: error, eventId: req.params.id }, '删除事件失败');
...
router.post('/insert-offline-blocks', async (req, res) => {
  try {
    ...
    logger.error({ err: error }, '插入离线时段失败');
...
router.post('/insert-rest-breaks', async (req, res) => {
  try {
    ...
    logger.error({ err: error }, '插入休息时段失败');
...
router.get('/analyze/:date', async (req, res) => {
  try {
    ...
    logger.error({ err: error, date: req.params.date }, '分析日程失败');
...
router.get('/day-schedule/:date', async (req, res) => {
  try {
    ...
    logger.error({ err: error, date: req.params.date }, '获取日程失败');
...
router.post('/suggest-meeting-time', async (req, res) => {
  try {
    ...
    logger.error({ err: error }, '建议会议时间失败');
