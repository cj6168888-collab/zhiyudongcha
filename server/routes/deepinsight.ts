...
const logger = createServiceLogger('Deepinsight');
...
export function registerDeepInsightRoutes(app: Express, storage: IStorage) {
  ...
  router.post('/dream/analyze', async (req, res) => {
    try {
      ...
      logger.error({ err: error }, 'Dream analysis error');
  ...
  router.get('/dream/graph', async (req, res) => {
    try {
      ...
      logger.error({ err: error }, 'Graph generation error');
  ...
  router.get('/dream/insights', async (req, res) => {
    try {
      ...
      logger.error({ err: error }, 'Insights error');
  ...
  router.post('/dream/force-cycle', async (req, res) => {
    try {
      ...
      logger.error({ err: error }, 'Force cycle error');
  ...
  router.post('/empathy/analyze', async (req, res) => {
    try {
      ...
      logger.error({ err: error }, 'Emotion analysis error');
  ...
  router.post('/executor/plan', async (req, res) => {
    try {
      ...
      logger.error({ err: error }, 'Plan creation error');
  ...
  router.post('/executor/execute/:planId', async (req, res) => {
    try {
      ...
      logger.error({ err: error }, 'Execution error');
  ...
  logger.info('[DeepInsight] Routes registered at /api/deepinsight/*');
}
...
export function registerChrysalisRoutes(app: Express) {
  ...
  logger.info('[Chrysalis] Routes registered at /api/chrysalis/*');
}
