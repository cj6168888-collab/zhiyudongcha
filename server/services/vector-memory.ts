...
const logger = createServiceLogger('VectorMemory');
...
class VectorMemoryService {
  ...
  async storeMemory(
    ...
    try {
      const [result] = await getDatabase().insert(shadowMemories).values({
      ...
      logger.info(`[VectorMemory] Stored memory #${result.id}: ${content.slice(0, 50)}...`);
      return result.id;
  ...
  async updateWeight(memoryId: string, newWeight: number): Promise<boolean> {
    try {
      await getDatabase().update(shadowMemories)
  ...
  async reinforce(memoryId: string, positive: boolean): Promise<void> {
    ...
    logger.info(`[VectorMemory] Reinforced #${memoryId}: ${currentWeight.toFixed(2)} -> ${newWeight.toFixed(2)}`);
  }
  ...
  async applyDecay(): Promise<number> {
    try {
      ...
      logger.info(`[VectorMemory] Applied decay to ${decayed} memories`);
      return decayed;
      
    } catch (error) {
      logger.error({ err: error }, 'Decay failed');
      return 0;
    }
  }
  ...
  async getStats(): Promise<{
    ...
    try {
      const memories = await getDatabase().select().from(shadowMemories);
      ...
      logger.error({ err: error }, 'Stats failed');
