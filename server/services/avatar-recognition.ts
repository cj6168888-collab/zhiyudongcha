...
const logger = createServiceLogger('AvatarRecognition');
...
class AvatarRecognitionService {
  ...
  constructor() {
    ...
    logger.info('[AvatarRecognition] 头像视觉识别服务已初始化');
  }
...
请用以下JSON格式返回（不要添加markdown代码块）：
{
  ...
  "greeting": "作为小智，一句温馨的问候语（用'爸爸'称呼）"
      ...
      if (!response.ok) {
        ...
        logger.error({ errorText }, '[AvatarRecognition] API error');
        return {
          success: false,
      ...
      try {
        ...
        logger.info('[AvatarRecognition] 分析成功:', analysis.description.substring(0, 50) + '...');
      ...
      logger.error({ err: error }, '[AvatarRecognition] Error');
      return {
        success: false,
