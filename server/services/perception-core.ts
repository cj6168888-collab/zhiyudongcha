...
const logger = createServiceLogger('PerceptionCore');
...
class PerceptionCoreService {
  ...
  async createSession(deviceId: string, userRole: 'MASTER' | 'GUEST' = 'MASTER'): Promise<PerceptionSession> {
    ...
    logger.info(`[感知核心] 新会话创建: ${sessionId} (设备: ${deviceId})`);
  ...
  async endSession(sessionId: string): Promise<boolean> {
    ...
    if (session) {
      ...
      logger.info(`[感知核心] 会话结束: ${sessionId}`);
...
只返回JSON。如果场景普通不需要响应，返回 {"type":"SILENT","urgency":"LOW","reasoning":"普通场景"}` },
      ...
      try {
        ...
        logger.error({ err: e }, '响应解析失败');
      }

      return { type: 'SILENT', urgency: 'LOW' };
    } catch (error) {
      logger.error({ err: error }, 'AI响应生成失败');
      return null;
    }
  }
  ...
  async sendResponseToGlasses(
    ...
    if (!targetClient) {
      logger.info(`[感知核心] 未找到设备 ${session.deviceId} 的连接`);
      return false;
    }
