...
const logger = createServiceLogger('OllamaAdapter');
...
export class OllamaAdapter {
  ...
  async checkHealth(): Promise<boolean> {
    ...
    try {
      ...
      if (this.isHealthy) {
        logger.info('[OllamaAdapter] Health check: ONLINE');
      } else {
        logger.info('[OllamaAdapter] Health check: UNHEALTHY');
      }
      ...
      this.lastHealthCheck = now;
      logger.info('[OllamaAdapter] Health check: OFFLINE');
      return false;
    }
  }
  ...
  async pullModel(modelName: string, onProgress?: (progress: number) => void): Promise<boolean> {
    try {
      ...
      logger.info(`[OllamaAdapter] Model pulled: ${modelName}`);
      return true;
    } catch (error) {
  ...
  async chat(
    ...
    if (!isHealthy) {
      logger.info('[OllamaAdapter] Cannot chat: Ollama is offline');
      return null;
    }
    ...
    for (let attempt = 0; attempt <= this.config.retries; attempt++) {
      try {
        ...
        const data = await response.json();
        logger.info(`[OllamaAdapter] Chat completed with ${model}`);
        
        return data as OllamaResponse;
      } catch (error) {
        lastError = error instanceof Error ? error : new Error('Unknown error');
        logger.info(`[OllamaAdapter] Chat attempt ${attempt + 1} failed: ${lastError.message}`);
    ...
    logger.error({ err: lastError }, 'Chat failed after retries');
    return null;
  }
  ...
  async *chatStream(
    ...
    const isHealthy = await this.checkHealth();
    if (!isHealthy) {
      logger.info('[OllamaAdapter] Cannot stream chat: Ollama is offline');
      return;
    }
    ...
    try {
      ...
      logger.error({ err: error }, 'Stream chat failed');
    }
  }
