...
const logger = createServiceLogger('FaceCompare');
...
class FaceCompareService {
  ...
  constructor() {
    ...
    logger.info('[FaceCompare] 人脸比对服务已初始化 (使用 Qwen-VL 双图比对)');
  }
  ...
  async compareFaces(imageA: string, imageB: string): Promise<FaceCompareResult> {
    ...
    for (let attempt = 0; attempt <= this.MAX_RETRIES; attempt++) {
      try {
        if (attempt > 0) {
          logger.info(`[FaceCompare] 重试第 ${attempt} 次...`);
        ...
        logger.error({ attempt: attempt + 1, error: lastError }, 'Face compare attempt failed');
      }
    }
...
- analysis: 字符串，判断依据`
    ...
    if (!response.ok) {
      ...
      logger.error({ status: response.status, errorText }, 'API error');
      return {
        success: false,
        isSamePerson: false,
        confidence: 0,
    ...
    if (!parsed) {
      logger.error({ contentPreview: content.substring(0, 200) }, 'Failed to parse/validate response');
      return {
        success: false,
        isSamePerson: false,
        confidence: 0,
...
请用以下JSON格式返回（不要添加markdown代码块）：
{
  ...
  "livenessCheck": "PASS"/"SUSPICIOUS"/"FAIL"
      ...
      logger.error({ err: error }, 'Quality check error');
      return {
        success: false,
        quality: 'LOW',
        hasFace: false,
