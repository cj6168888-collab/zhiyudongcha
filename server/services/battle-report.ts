...
class BattleReportService {
  ...
  async generateDailySummary(date?: Date): Promise<{
    ...
    const activeDevices = await getDatabase().select({ count: sql<number>`count(*)` })
  ...
  async getDeviceActivitySummary(): Promise<DeviceActivity[]> {
    ...
    for (const device of devices) {
      const reports = await getDatabase().select({ count: sql<number>`count(*)` })
        .from(battleReports)
        .where(eq(battleReports.sourceDeviceId, device.id));

      const lastReport = await getDatabase().select()
        .from(battleReports)
        .where(eq(battleReports.sourceDeviceId, device.id))
  ...
  async assessSecurityStatus(): Promise<SecurityAssessment> {
    ...
    const recentLoyaltyEvents = await getDatabase().select()
      .from(loyaltyEvents)
      .where(and(
    ...
    const killedDevices = await getDatabase().select()
      .from(satelliteDevices)
    ...
    const weeklyReports = await getDatabase().select()
      .from(battleReports)
    ...
    const restrictedMembers = await getDatabase().select()
      .from(teamMembers)
  ...
  async submitBattleReport(data: {
    ...
    const [report] = await getDatabase().insert(battleReports).values({
