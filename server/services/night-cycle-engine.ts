...
const logger = createServiceLogger('NightCycleEngine');
...
class NightCycleEngineService {
  ...
  private transitionToDream(): void {
    logger.info('[NightCycle] Transitioning to DREAM mode...');
    this.currentMode = 'TRANSITIONING';
    this.lastModeChange = new Date();
    
    this.prepareDreamSession().then(() => {
      this.currentMode = 'DREAM';
      logger.info('[NightCycle] DREAM mode active. Starting dream tasks...');
      this.startDreamSession();
    });
  }
  
  private transitionToActive(): void {
    logger.info('[NightCycle] Transitioning to ACTIVE mode...');
    this.currentMode = 'TRANSITIONING';
    this.lastModeChange = new Date();
    ...
    logger.info('[NightCycle] ACTIVE mode restored.');
  }
  ...
  private async prepareDreamSession(): Promise<void> {
    ...
    logger.info(`[NightCycle] Prepared ${this.dreamTaskQueue.length} dream tasks`);
  }
  ...
  private async startDreamSession(): Promise<void> {
    ...
    try {
      ...
      logger.info(`[NightCycle] Dream session started: ${dreamLog.id}`);
      
      for (const task of this.dreamTaskQueue) {
        if (this.currentMode !== 'DREAM') {
          logger.info('[NightCycle] Dream session interrupted by mode change');
          break;
        }
      ...
      await getDatabase().update(dreamLogs)
        .set({
          status: 'AWAKENED',
          durationMs,
      ...
      logger.info(`[NightCycle] Dream session completed. Duration: ${durationMs}ms`);
      
    } catch (error) {
      logger.error({ err: error }, 'Dream session error');
    } finally {
  ...
  private pauseDreamSession(): void {
    logger.info('[NightCycle] Pausing dream session...');
    this.dreamSessionActive = false;
  }
  ...
  private async executeDreamTask(task: DreamTask): Promise<void> {
    ...
    logger.info(`[NightCycle] Executing dream task: ${task.type}`);
    ...
    try {
      ...
      logger.info(`[NightCycle] Task completed: ${task.type}`);
      
    } catch (error) {
      task.status = 'failed';
      task.error = error instanceof Error ? error.message : String(error);
      logger.error({ err: error, taskType: task.type }, 'Task failed');
    }
  }
  
  private async executeMemoryConsolidation(input: Record<string, unknown>): Promise<Record<string, unknown>> {
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    
    const chatHistory = await getDatabase().select()
      .from(avatarChatHistory)
      .where(gte(avatarChatHistory.createdAt, today))
      .orderBy(desc(avatarChatHistory.createdAt));
    
    const auditEntries = await getDatabase().select()
      .from(auditLogs)
  ...
  private analyzeMoodFromChats(chats: Array<{ content?: string | null }>): { trend: string; insights: string[] } {
  ...
  private extractRelationshipMentions(chats: Array<{ content?: string | null }>): string[] {
  ...
  private identifyPainPoints(audits: Array<{ result?: string | null; action?: string | null }>): string[] {
  ...
  private async executeBusinessAnalysis(input: Record<string, unknown>): Promise<Record<string, unknown>> {
    const since = new Date();
    const analyzeLastDays = typeof input.analyzeLastDays === 'number' ? input.analyzeLastDays : 1;
    since.setDate(since.getDate() - analyzeLastDays);
  ...
  private async executeSelfImprovement(input: Record<string, unknown>): Promise<Record<string, unknown>> {
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    
    const failedTasks = await getDatabase().select()
      .from(auditLogs)
      .where(and(
  ...
  private async executeEmotionalAlignment(input: Record<string, unknown>): Promise<Record<string, unknown>> {
    const chatHistory = await getDatabase().select()
      .from(avatarChatHistory)
      .orderBy(desc(avatarChatHistory.createdAt))
      .limit(100);
  ...
  async forceDreamCycle(): Promise<DreamSessionResult> {
    logger.info('[NightCycle] Force starting dream cycle...');
