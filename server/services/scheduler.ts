...
class SchedulerService {
  ...
  private getDefaultJobs(): JobConfig[] {
    return [
      ...
      },
      {
        ...
        jobName: '晨间建议推送',
        description: '每日早晨6点生成醒来建议并推送给在线用户',
        cronExpression: '0 6 * * *',
        handler: this.handleMorningBriefing.bind(this),
        maxRetries: 2,
      },
    ];
  }
  ...
  private async startAllJobs(): Promise<void> {
    ...
    for (const job of jobs) {
      ...
      if (!handler) {
        logger.warn(`[Scheduler] 未找到任务处理器: ${job.jobType}`);
        continue;
      }
  ...
  private scheduleJob(config: JobConfig): void {
    ...
    logger.info(`[Scheduler] 已调度: ${config.jobName} (${config.cronExpression})`);
  }
  ...
  private async executeJob(config: JobConfig, attemptNumber: number = 1): Promise<void> {
    ...
    try {
      ...
      logger.info(`[Scheduler] 执行任务: ${config.jobName} (尝试 ${attemptNumber})`);
      ...
      logger.info(`[Scheduler] 任务完成: ${config.jobName} (${durationMs}ms)`);
    } catch (error) {
      const completedAt = new Date();
      const durationMs = completedAt.getTime() - startedAt.getTime();
      ...
      logger.error({ error: errorMessage, jobName: config.jobName }, '任务失败');

      if (willRetry) {
        const retryDelayMs = config.retryDelayMs || 60000;
        logger.info(`[Scheduler] ${retryDelayMs / 1000}秒后重试...`);
        setTimeout(() => {
  ...
  private async handleDreamConsolidation(): Promise<Record<string, unknown>> {
    logger.info('[DreamConsolidation] 开始梦境整理 (Phase 3.1)...');

    const result: {
      consolidatedCount: number;
      newInsights: unknown[];
      dreamReport: Record<string, unknown> | null;
      dreamLogId?: string;
    } = {
      consolidatedCount: 0,
      newInsights: [],
    ...
    try {
      ...
      if (dreamModule?.dreamService) {
        ...
        logger.info(`[DreamConsolidation] 梦境日志已生成: ${dreamResult.dreamLogId}`);
      } else {
        logger.info('[DreamConsolidation] DreamService不可用，尝试旧版Chrysalis');
      ...
      logger.error({ err: error }, '梦境整理失败');
    }

    logger.info(`[DreamConsolidation] 整理完成: ${result.consolidatedCount}条对话，${result.newInsights.length}条洞察`);
    return result;
  }

  private async handleProactiveCare(): Promise<Record<string, unknown>> {
    logger.info('[ProactiveCare] 执行主动关怀扫描...');
    ...
    try {
      ...
      if (careModule?.proactiveCareService) {
        ...
        logger.info(`[ProactiveCare] 扫描完成: 检测${scanResult.eventsDetected}个事件, 触发${scanResult.rulesTriggered}条规则, 队列${scanResult.notificationsQueued}条通知`);
        return scanResult;
      } else {
        logger.info('[ProactiveCare] ProactiveCareService不可用');
        return { eventsDetected: 0, rulesTriggered: 0, notificationsQueued: 0 };
      }
    } catch (error) {
      logger.error({ err: error }, '主动关怀扫描失败');
      return { eventsDetected: 0, rulesTriggered: 0, notificationsQueued: 0, error: String(error) };
    }
  }

  private async handleReminderTrigger(): Promise<Record<string, unknown>> {
    logger.info('[ReminderTrigger] 检查待触发提醒...');
    ...
    try {
      ...
      logger.info('[ReminderTrigger] 日历服务不可用，跳过');
    }

    logger.info(`[ReminderTrigger] 检查完成: ${result.checkedCount}个事件`);
    return result;
  }

  private async handleMemoryCleanup(): Promise<Record<string, unknown>> {
    logger.info('[MemoryCleanup] 开始清理过期记忆...');

    const result = {
      cleanedCount: 0,
    };

    logger.info('[MemoryCleanup] 清理完成');
    return result;
  }

  private async handleHealthSync(): Promise<Record<string, unknown>> {
    logger.info('[HealthSync] 同步健康数据...');

    const result = {
      syncedRecords: 0,
    };

    logger.info('[HealthSync] 同步完成');
    return result;
  }

  private async handleMorningBriefing(): Promise<Record<string, unknown>> {
    logger.info('[MorningBriefing] 生成晨间建议...');

    const db = getDatabase();
    if (!db) {
      logger.warn('[MorningBriefing] DB unavailable');
      return { skipped: true };
    }
    ...
    try {
      ...
      const ownerIds: string[] = (rows.rows ?? []).map((r: Record<string, unknown>) => r.owner_id as string);

      const { morningBriefingService } = await import('./reflection/MorningBriefingService');
      let pushed = 0;

      for (const ownerId of ownerIds) {
        const briefing = await morningBriefingService.generateBriefing(ownerId);
        if (briefing) {
          webSocketManager.context.broadcastDataChange(
            'morning_briefing',
            'CREATE',
      ...
      logger.info(`[MorningBriefing] 完成：处理 ${ownerIds.length} 位用户，推送 ${pushed} 条`);
      return { ownersProcessed: ownerIds.length, pushed };
    } catch (err) {
      logger.error({ err }, '[MorningBriefing] 生成失败');
