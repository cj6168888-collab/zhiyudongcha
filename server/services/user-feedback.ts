...
const logger = createServiceLogger('UserFeedback');
...
class UserFeedbackService {
  ...
  async recordFeedback(
    ...
    try {
      // 记录到审计日志
      const [result] = await getDatabase().insert(auditLogs).values({
      ...
      logger.info(`[Feedback] Recorded: ${taskType} -> ${sentiment}`);
      return result.id;
  ...
  async getTaskPerformance(
    ...
    try {
      ...
      logger.error({ err: error }, 'Failed to get performance');
      return {
        taskType,
        totalCount: 0,
        positiveCount: 0,
  ...
  async getAllPerformance(days: number = 30): Promise<TaskPerformance[]> {
    ...
    try {
      ...
      logger.error({ err: error }, 'Failed to get all performance');
      return [];
    }
  }
  ...
  async generateEvolutionReport(days: number = 7): Promise<EvolutionReport> {
    ...
    // 保存报告
    try {
      await getDatabase().insert(dailyReports).values({
        reportDate: new Date(),
