...
const logger = createServiceLogger('DesktopExecutor');
...
class DesktopExecutorService {
  ...
  async createExecutionPlan(name: string, description: string, actions: Omit<ExecutionAction, 'id'>[]): Promise<ExecutionPlan> {
    ...
    logger.info(`[DesktopExecutor] Plan created: ${plan.id} with ${actions.length} actions`);
  ...
  private async executeDesktopAction(action: ExecutionAction): Promise<ExecutionResult> {
    ...
    logger.info(`[DesktopExecutor] Desktop action: ${action.actionType}`, action.params);
  ...
  private async executeAndroidAction(action: ExecutionAction): Promise<ExecutionResult> {
    ...
    logger.info(`[DesktopExecutor] ADB action: ${action.actionType}`, action.params);
  ...
  private async executeWebAction(action: ExecutionAction): Promise<ExecutionResult> {
    ...
    logger.info(`[DesktopExecutor] Web action: ${action.actionType}`, action.params);
  ...
  async executePlan(planId: string): Promise<ExecutionPlan> {
    ...
    logger.info(`[DesktopExecutor] Executing plan: ${plan.name}`);
    ...
    logger.info(`[DesktopExecutor] Plan ${plan.status}: ${plan.name}`);
