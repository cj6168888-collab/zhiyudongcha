...
只返回JSON，不要其他内容。`;
    ...
    try {
      ...
      logger.error({ err: error }, '[CapabilityIndexer] AI extraction failed');
    }
  ...
  async indexPerson(personId: string): Promise<void> {
    const [person] = await getDatabase().select().from(persons).where(eq(persons.id, personId)).limit(1);

    if (!person) {
      logger.error({ personId }, '[CapabilityIndexer] Person not found');
      return;
    }

    logger.info(`[CapabilityIndexer] Indexing capabilities for: ${person.name}`);

    const extraction = await this.extractCapabilitiesFromPerson(person);

    await getDatabase().update(persons)
      .set({
    ...
    logger.info(`[CapabilityIndexer] Indexed ${extraction.capabilities.length} capabilities for ${person.name}`);
  }
  ...
  private async updateCapabilityIndex(
    ...
    const [existing] = await getDatabase().select().from(teamCapabilityIndex).where(eq(teamCapabilityIndex.capability, capability)).limit(1);
    ...
    if (existing) {
      ...
      await getDatabase().update(teamCapabilityIndex)
        .set({
          personIds,
          personNames,
          totalCount,
      ...
      await getDatabase().insert(teamCapabilityIndex).values({
        capability,
        category,
        personIds: [personId],
  ...
  async indexAllPersons(): Promise<{ indexed: number; failed: number }> {
    const allPersons = await getDatabase().select().from(persons);
    let indexed = 0;
    let failed = 0;
    ...
    for (const person of allPersons) {
      try {
        ...
        logger.error({ err: error, personName: person.name }, '[CapabilityIndexer] Failed to index');
        failed++;
      }
    }
  ...
  async searchByCapability(query: string): Promise<Array<{
    ...
    const matchingIndices = await getDatabase().select().from(teamCapabilityIndex).where(ilike(teamCapabilityIndex.capability, `%${query}%`));
  ...
  async getTeamCapabilityOverview(): Promise<{
    ...
    const allIndices = await getDatabase().select().from(teamCapabilityIndex);
  ...
  async findPersonsWithCapabilities(requiredCapabilities: string[]): Promise<Array<{
    ...
    for (const cap of requiredCapabilities) {
      const [index] = await getDatabase().select().from(teamCapabilityIndex).where(ilike(teamCapabilityIndex.capability, `%${cap}%`)).limit(1);
