...
function parseJSONResponse(text: string): ParsedEmailAnalysis | null {
  const jsonMatch = text.match(/\{[\s\S]*\}/);
  if (jsonMatch) {
    try {
      return JSON.parse(jsonMatch[0]) as ParsedEmailAnalysis;
    } catch (e) {
...
export class EmailIntelligenceService {
  ...
  private validateUrgencyLevel(level: string): EmailAnalysis['urgencyLevel'] {
    const validLevels = ['low', 'normal', 'high', 'urgent'] as const;
    return validLevels.includes(level as string) ? level as EmailAnalysis['urgencyLevel'] : 'normal';
  }
  ...
  async analyzeEmail(email: Email): Promise<EmailAnalysis> {
    ...
    try {
      ...
      const analysis: EmailAnalysis = {
        ...
        urgencyLevel: this.validateUrgencyLevel(parsed.urgencyLevel || 'normal'),
        emotionalTone: parsed.emotionalTone || '中性',
        keyPoints: parsed.keyPoints || [],
        suggestedResponses: (parsed.suggestedResponses || []).map((r) => ({
          type: (r.type || 'clarify') as SuggestedResponse['type'],
          label: r.label || '回复',
          script: r.script || '',
          tone: r.tone || '礼貌',
          riskLevel: (r.riskLevel || 'low') as SuggestedResponse['riskLevel'],
        })),
        actionItems: (parsed.actionItems || []).map((a) => ({
          description: a.description || '',
          deadline: a.deadline || null,
          priority: (a.priority || 'normal') as ActionItem['priority'],
          category: (a.category || 'other') as ActionItem['category'],
        })),
      ...
      logger.error({ err: error, emailId: email.id }, 'Analysis failed');
      return this.createFallbackAnalysis(email);
    }
  }
    ...
    try {
      ...
      if (parsed?.attachments) {
        return parsed.attachments.map((a) => ({
          filename: a.filename || '',
          type: (a.type || 'other') as AttachmentClassification['type'],
  ...
  private createFallbackAnalysis(email: Email): EmailAnalysis {
    return {
      ...
      hiddenIntent: null,
      urgencyLevel: this.validateUrgencyLevel(email.importance?.toString() || 'normal'),
      emotionalTone: '中性',
      keyPoints: [],
      suggestedResponses: [{
        type: 'clarify',
  ...
  async batchAnalyze(emails: Email[]): Promise<Map<string, EmailAnalysis>> {
    ...
    for (const email of emails) {
      try {
        ...
        logger.error({ err: error, emailId: email.id }, 'Failed to analyze email');
      }
    }
