...
const logger = createServiceLogger('ContactRecognition');
...
class ContactRecognitionService {
  ...
  constructor() {
    ...
    logger.info('[ContactRecognition] 联系人识别服务已初始化');
  }
  ...
  async enrollFace(personId: string, faceImage: string, notes?: string): Promise<EnrollResult> {
    ...
    // 存入数据库
    const [result] = await getDatabase().insert(contactBiometrics).values({
      personId,
      biometricType: 'FACE',
      featureHash,
      featureDescription,
    ...
    logger.info(`[ContactRecognition] 已为联系人 ${personInfo.name} 录入人脸特征`);
...
请用简洁的文字描述，控制在200字以内。`
      ...
      logger.error({ err: error }, '[ContactRecognition] 特征提取失败');
      return null;
    }
  }
  ...
  async recognizeFace(faceImage: string, threshold: number = 60): Promise<RecognitionResult> {
    ...
    for (const record of storedBiometrics) {
      ...
      if (compareResult && compareResult.similarityScore >= threshold) {
        ...
        // 更新匹配计数
        await getDatabase().update(contactBiometrics)
          .set({
...
- 需要综合考虑脸型、五官、肤色等多个维度`
      ...
      logger.error({ err: error }, '[ContactRecognition] 比对失败');
      return null;
    }
  }
  ...
  async deleteBiometric(biometricId: string): Promise<boolean> {
    try {
      ...
      logger.error({ err: error }, '[ContactRecognition] 删除失败');
      return false;
    }
  }
  ...
  async refreshCache(): Promise<void> {
    ...
    logger.info(`[ContactRecognition] 缓存已刷新，共 ${records.length} 条记录`);
  }
