...
export function createSession(
  ...
  logger.info(`[MockOp] Session created: ${sessionId} for device ${deviceId}`);
...
export function closeSession(sessionId: string): boolean {
  ...
  if (session) {
    ...
    logger.info(`[MockOp] Session closed: ${sessionId}`);
    return true;
  }
  return false;
}
...
export function queueAction(sessionId: string, action: ScreenAction): boolean {
  ...
  if (!session || !session.connected) {
    logger.error(`[MockOp] Cannot queue action: session ${sessionId} not found or disconnected`);
    return false;
  }
  
  session.pendingActions.push(action);
  logger.info(`[MockOp] Action queued: ${action.type} -> ${action.target}`);
  return true;
}
...
export function completeAction(
  ...
  if (actionIndex >= 0) {
    ...
    logger.info(`[MockOp] Action ${result.success ? 'completed' : 'failed'}: ${action.type} -> ${action.target}`);
    return true;
  }
...
const logger = createServiceLogger('MockOpLayer');
