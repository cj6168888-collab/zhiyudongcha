...
const logger = createServiceLogger('PluginSystem');
...
class PluginSystemService {
  ...
  async createLogicPackage(params: {
    ...
    logger.info(`[PluginSystem] 创建逻辑指令包: ${params.name}`);
    return plugin;
  }
  ...
  async createKnowledgePatch(params: {
    ...
    this.plugins.set(id, plugin);
    logger.info(`[PluginSystem] 创建知识库补丁: ${params.name} (${params.domain})`);
    return plugin;
  }
  ...
  async createBehaviorProfile(params: {
    ...
    this.plugins.set(id, plugin);
    logger.info(`[PluginSystem] 创建行为配置: ${params.name}`);
    return plugin;
  }
  ...
  async validatePlugin(pluginId: string): Promise<ValidationResult> {
    ...
    logger.info(`[PluginSystem] 验证插件: ${plugin.name}`);
    ...
    logger.info(`[PluginSystem] 验证结果: ${passed ? '通过' : '失败'} (${score * 100}%)`);
  ...
  async activatePlugin(pluginId: string): Promise<boolean> {
    ...
    logger.info(`[PluginSystem] 激活插件: ${plugin.name}`);
  ...
  async deactivatePlugin(pluginId: string): Promise<boolean> {
    ...
    logger.info(`[PluginSystem] 停用插件: ${plugin.name}`);
  ...
  async rollbackPlugin(pluginId: string, reason: string): Promise<boolean> {
    ...
    logger.info(`[PluginSystem] 回滚插件: ${plugin.name}, 原因: ${reason}`);
