...
const logger = createServiceLogger('DreamAnalyzer');
...
export interface AnalysisResult {
  ...
  personMentions?: Record<string, number>;
...
class DreamAnalyzerService {
  ...
  async analyzeWithQwenMax(
    ...
    try {
      ...
      logger.error({ error }, 'Qwen-Max analysis failed');
...
输出格式：
{
  ...
  private formatMemoriesForAnalysis(memories: MemoryItem[]): string {
  ...
  private parseAnalysisResponse(response: string, type: string): AnalysisResult {
    try {
      ...
      logger.warn('[DreamAnalyzer] Failed to parse JSON response, using text extraction');
    }
  ...
  private fallbackAnalysis(memories: MemoryItem[], type: string): AnalysisResult {
    logger.info('[DreamAnalyzer] Using fallback local analysis');
  ...
  async generateRelationshipGraph(): Promise<RelationshipGraph> {
    const personsData = await getDatabase().select().from(persons).limit(100);
    const projectsData = await getDatabase().select().from(projects).limit(50);
  ...
  private identifyClusters(
    ...
    for (const node of nodes) {
      if (node.type === 'person' && node.attributes.organization) {
        const org = String(node.attributes.organization);
  ...
  async generateDeepInsights(): Promise<DeepInsight[]> {
    ...
    const recentAudits = await getDatabase().select()
      .from(auditLogs)
  ...
  async runComprehensiveAnalysis(): Promise<DreamAnalysisResult> {
    logger.info('[DreamAnalyzer] Starting comprehensive analysis...');
    ...
    const recentMemoriesRaw = await getDatabase().select()
      .from(shadowMemories)
      .orderBy(desc(shadowMemories.createdAt))
      .limit(100);
    
    const recentMemories: MemoryItem[] = recentMemoriesRaw.map(m => ({
      content: m.context || undefined,
      choiceMade: m.choiceMade || undefined,
      mimicryWeight: m.mimicryWeight || undefined,
    }));
    ...
    logger.info('[DreamAnalyzer] Comprehensive analysis complete');
    return result;
  }
  
  encryptMemoryPacket(data: unknown): EncryptedMemoryPacket {
  ...
  decryptMemoryPacket(packet: EncryptedMemoryPacket): unknown {
