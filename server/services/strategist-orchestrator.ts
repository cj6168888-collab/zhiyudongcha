...
const logger = createServiceLogger('StrategistOrchestrator');
...
interface ParsedProposalData {
  ...
  proposalType?: string;
  urgency?: string;
  recommendedActions?: ProposalAction[];
  alternativeOptions?: AlternativeOption[];
  estimatedROI?: number | null;
  riskAssessment?: { overallRisk: string; factors: string[] } | null;
}
  ...
  try {
    ...
    if (Array.isArray(parsed)) {
      for (const item of parsed) {
        const opp = item as ParsedOpportunityData;
    ...
    logger.error({ err: error }, '商机扫描失败');
  }
  
  logger.info(`[Strategist] 扫描完成，发现 ${detectedOpportunities.length} 个商机`);
...
export async function triggerRefinement(opportunityId: string, runType: 'DREAM' | 'QUICK_ANALYSIS' | 'DEEP_DIVE' = 'DREAM'): Promise<RefinementRun> {
  logger.info(`[Strategist] 启动推演: ${opportunityId}, 类型: ${runType}`);
...
export async function generateProposal(opportunityId: string, refinementRunId?: string): Promise<StrategyProposal> {
  logger.info(`[Strategist] 生成提案: ${opportunityId}`);
...
请生成详细的策略提案。`;
  ...
  try {
    ...
    const rawParsed = parseJsonFromResponse(response);
    const parsed = rawParsed as ParsedProposalData | null;
    ...
    logger.info(`[Strategist] 提案生成成功: ${proposal.id}`);
    return proposal;
  } catch (error) {
    logger.error({ err: error }, '提案生成失败');
    throw error;
  }
}
...
export async function recordUserDecision(
  ...
  feedback?: string
): Promise<{ proposal: StrategyProposal; alignmentSignal: AlignmentSignal }> {
  logger.info(`[Strategist] 记录用户决策: ${proposalId} -> ${decision}`);
  ...
  logger.info(`[Strategist] 对齐信号已记录: ${alignmentSignal.id}, 权重调整: ${weightAdjustment}`);
...
export async function getStrategistDashboard(): Promise<{
  ...
  recentAlignmentSignals: AlignmentSignal[];
  stats: {
...
export async function learnFromAlignmentSignals(): Promise<{
  ...
  logger.info('[Strategist] 开始学习对齐信号...');
  ...
  for (const signal of unprocessedSignals) {
    await storage.updateAlignmentSignal(signal.id, { weightAdjustment: signal.weightAdjustment });
  }
  
  logger.info(`[Strategist] 处理完成: ${unprocessedSignals.length} 个信号`);
