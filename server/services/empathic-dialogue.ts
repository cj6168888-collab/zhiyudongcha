...
const logger = createServiceLogger('EmpathicDialogue');
...
class EmpathicDialogueService {
  ...
  async buildDialogueContext(userId: string = 'master'): Promise<DialogueContext> {
    ...
    try {
      ...
      logger.error({ err: error }, '[EmpathicDialogue] Error building context');
      return {
        recentMessages: [],
  ...
  async recordEmotionalInteraction(
    ...
    try {
      ...
      logger.error({ err: error }, '[EmpathicDialogue] Error recording interaction');
    }
  }
