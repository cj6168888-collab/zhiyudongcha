    ...
    return {
      ...
      chainOfThought: [
        ...
        },
        {
          ...
          conclusion: '高比例日违约金不应写成当然无效，而应表述为存在较高被调低或不被全部支持的争议风险；关键取决于实际损失、履行情况、过错和证据。',
          confidence: 90,
        },
        {
          step: 3,
      ...
      finalVerdict: '建议重点修改三处：第一，将高比例日违约金改为“按未履行部分或逾期金额的一定比例计算，并设置总额上限，例如不超过合同总价10%-20%，且以实际损失为调整基础”；第二，将单方不便利管辖改为合同履行地、乙方所在地或中立地法院/仲裁机构；第三，将单方验收或拒付尾款改为双方书面验收、明确异议期限、逾期视为验收、无争议部分按期付款。《民法典》第五百八十五条支持对过高违约金请求调整，其他程序性条款仍需结合完整合同文本复核。',
      riskLevel: 'HIGH',
      recommendations: [
        '把违约金条款改成：逾期违约金以实际损失为基础，按未交付部分价款每日0.05%-0.1%计算，累计不超过合同总价10%-20%，明显过高时双方可协商或依法请求调整。',
