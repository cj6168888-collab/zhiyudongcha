...
class ContractPipelineService {
  ...
  async initialize(): Promise<void> {
    ...
    logger.info('[ContractPipeline] Service initialized');
  }

  private async seedDefaultTemplates(): Promise<void> {
    const existing = await getDatabase().select().from(contractTemplates).limit(1);
    
    if (existing.length === 0) {
      logger.info('[ContractPipeline] Seeding default templates...');
      
      for (const template of DEFAULT_TEMPLATES) {
        await getDatabase().insert(contractTemplates).values(template as InsertContractTemplate);
      }
      
      logger.info(`[ContractPipeline] Seeded ${DEFAULT_TEMPLATES.length} default templates`);
    }
  }
  ...
  async getTemplates(options?: {
    ...
    let query = getDatabase().select().from(contractTemplates).where(eq(contractTemplates.isActive, true));
  ...
  async createTemplate(input: InsertContractTemplate): Promise<ContractTemplate> {
    const [template] = await getDatabase().insert(contractTemplates)
      .values(input)
      .returning();
    
    logger.info(`[ContractPipeline] Template created: ${template.name}`);
    return template;
  }
  ...
  async generateDraft(request: ContractRequest): Promise<ContractOutput> {
    ...
    const [draft] = await getDatabase().insert(contractDrafts).values({
    ...
    await getDatabase().update(contractTemplates)
      .set({ usageCount: sql`${contractTemplates.usageCount} + 1` })
      .where(eq(contractTemplates.id, template.id));

    logger.info(`[ContractPipeline] Draft generated: ${draft.id}`);
  ...
  async getDrafts(options?: {
    ...
    let query = getDatabase().select().from(contractDrafts);
  ...
  async getDraft(id: string): Promise<ContractDraft | null> {
    const [draft] = await getDatabase().select()
      .from(contractDrafts)
  ...
  async updateDraft(id: string, updates: Partial<InsertContractDraft>): Promise<ContractDraft | null> {
    const [updated] = await getDatabase().update(contractDrafts)
  ...
  async deleteDraft(id: string): Promise<boolean> {
    await getDatabase().delete(contractDrafts).where(eq(contractDrafts.id, id));
    return true;
  }
  ...
  async getStats(): Promise<{
    ...
    const templates = await getDatabase().select().from(contractTemplates);
    const drafts = await getDatabase().select().from(contractDrafts);
