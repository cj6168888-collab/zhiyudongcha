...
const logger = createServiceLogger('MobileEdgeAi');
...
export class MobileEdgeAI {
   ...
   */
  private constructor() {
    MobileEdgeAI.initializationCount++;
    
    if (MobileEdgeAI.initializationCount > 1) {
      logger.warn(
        { 
          count: MobileEdgeAI.initializationCount,
          stackTrace: new Error().stack 
        },
        '[MobileEdgeAI] instantiated multiple times - this indicates a code smell'
      );
    }
    
    logger.info(
      { instanceId: this.getInstanceId() },
      '[MobileEdgeAI] 移动边缘AI服务实例创建'
    );
  }
  ...
  public static getInstance(): MobileEdgeAI {
    if (!MobileEdgeAI.instance) {
      ...
      logger.info(
        { 
          initializationCount: MobileEdgeAI.initializationCount,
          timestamp: new Date().toISOString()
        },
        '[MobileEdgeAI] 单例实例初始化'
      );
    }
  ...
  public async initialize(): Promise<void> {
    if (this.initialized) {
      logger.debug('[MobileEdgeAI] 已经初始化，跳过');
      return;
    }
    
    logger.info('[MobileEdgeAI] 开始初始化移动边缘AI服务...');
    
    // 初始化逻辑（如果需要）
    this.initialized = true;
    
    logger.info('[MobileEdgeAI] 移动边缘AI服务已启动');
  }
  ...
  private getInstanceId(): string {
    return `MobileEdgeAI_${Date.now()}_${Math.random().toString(36).substring(7)}`;
  }
  ...
  public static resetInstance(): void {
    ...
    if (MobileEdgeAI.instance) {
      logger.warn('[MobileEdgeAI] 重置单例实例');
    }
    
    MobileEdgeAI.instance = null;
    MobileEdgeAI.initializationCount = 0;
  }
  ...
  public async healthCheck(): Promise<{
    ...
    const details = {
      ...
      instanceCount: MobileEdgeAI.initializationCount,
      instanceId: this.getInstanceId()
    };
    ...
    if (!this.initialized) {
      status = 'error';
    } else if (this.mobileDevices.size === 0) {
      status = 'degraded';
    } else {
      status = 'ok';
    }
  ...
  registerMobileDevice(config: MobileDeviceConfig, userId: string): {
    ...
    try {
      ...
      logger.info(`[MobileEdgeAI] 移动设备已注册: ${config.deviceName}`);
      logger.info(`[MobileEdgeAI] RAM: ${config.ramGB}GB, 芯片: ${chipInfo?.name || '未指定'}, 推荐模型: ${profile.recommendedModels.join(', ')}`);
      ...
      logger.error({ err: error }, '[MobileEdgeAI] 设备注册失败');
      return {
        success: false,
  ...
  async requestMobileInference(
    ...
    if (config?.tunnelUrl || config?.localEndpoint) {
      try {
        ...
        logger.error({ err: error }, '[MobileEdgeAI] 移动端推理失败');
  ...
  private addToOfflineQueue(request: MobileInferenceRequest): void {
    ...
    logger.info(`[MobileEdgeAI] 请求已加入离线队列: ${request.requestId}`);
  }
