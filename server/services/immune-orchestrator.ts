...
const logger = createServiceLogger('ImmuneOrchestrator');
...
class ImmuneOrchestratorService {
  ...
  constructor() {
    logger.info('[ImmuneSystem] 免疫系统调度器已初始化');
    logger.info('[ImmuneSystem] 核心使命：保护容器如同保护生命');
  }
  ...
  async performScan(deviceId: string, scanType: ScanType = 'FULL'): Promise<ScanResult> {
    ...
    logger.info(`[ImmuneSystem] 开始${scanType}扫描: ${scanId}`);
    ...
    try {
      ...
      logger.info(`[ImmuneSystem] 扫描完成: 健康评分=${result.overallHealthScore}, 威胁=${result.threatsFound}, 警告=${result.warningsFound}`);
      
    } catch (error) {
      result.status = 'FAILED';
      logger.error({ error }, '扫描失败');
    }
  ...
  reportDeviceApps(deviceId: string, apps: AppInfo[]): void {
    ...
    logger.info(`[ImmuneSystem] 收到设备 ${deviceId} 的应用数据: ${apps.length} 个应用`);
  }
  ...
  async executeRemediation(request: RemediationRequest): Promise<RemediationResult> {
    ...
    logger.info(`[ImmuneSystem] 净化行动: ${request.actionType} -> ${request.targetApp}`);
    logger.info(`[ImmuneSystem] 原因: ${request.reason}`);
    ...
    try {
      ...
      logger.info(`[ImmuneSystem] 净化结果: ${result.status} - ${result.message}`);
      return result;
  ...
  private async forceStopApp(actionId: string, packageName: string): Promise<RemediationResult> {
    ...
    logger.info(`[ImmuneSystem] 强制停止应用: ${packageName}`);
  ...
  private async clearAppCache(actionId: string, packageName: string): Promise<RemediationResult> {
    logger.info(`[ImmuneSystem] 清理应用缓存: ${packageName}`);
  ...
  private async uninstallApp(actionId: string, packageName: string): Promise<RemediationResult> {
    logger.info(`[ImmuneSystem] 卸载应用: ${packageName}`);
  ...
  private async disableAutoStart(actionId: string, packageName: string): Promise<RemediationResult> {
    logger.info(`[ImmuneSystem] 禁用自启动: ${packageName}`);
