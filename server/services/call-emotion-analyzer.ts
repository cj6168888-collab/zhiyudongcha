...
const logger = createServiceLogger('CallEmotionAnalyzer');
...
class CallEmotionAnalyzerService {
  ...
  private calculateConfidenceLevel(data: {
    preCallHeartRate?: number;
    duringCallHeartRate?: number;
    postCallHeartRate?: number;
    voiceToneData?: VoiceToneAnalysis;
  ...
  private generateInsights(data: {
    ...
    callDirection: 'INCOMING' | 'OUTGOING';
    callDuration: number;
  }, score: number, heartRateChange: number): string[] {
  ...
  private async logCallEmotion(data: InsertCallEmotionLog): Promise<CallEmotionLog> {
    const [log] = await getDatabase().insert(callEmotionLogs).values(data).returning();
    logger.info(`[CallEmotionAnalyzer] 通话情绪已记录: ${data.contactName}, 评分: ${data.emotionScore}`);
    return log;
  }
  
  async getContactEmotionProfile(contactName: string): Promise<ContactEmotionProfile | null> {
    const logs = await getDatabase().select().from(callEmotionLogs)
  ...
  async getRecentCallEmotions(days: number = 7): Promise<CallEmotionLog[]> {
    ...
    return await getDatabase().select().from(callEmotionLogs)
