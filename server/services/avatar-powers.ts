...
const logger = createServiceLogger('AvatarPowers');
...
export async function invokeOraclePower(): Promise<OracleInsight[]> {
  ...
  try {
    ...
    if (persons.length === 0) {
      ...
      const zoneRedCount = persons.filter(p => 'accessLevel' in p && p.accessLevel === 'ZONE_RED').length;
    ...
    if (projects.length > 0) {
      const planningProjects = projects.filter((p) => 'status' in p && p.status === 'PENDING_REVIEW');
    ...
    if (vaultItems.length > 0) {
      const sandboxedItems = vaultItems.filter((v) => 'sandboxStatus' in v && v.sandboxStatus === 'SANDBOXED');
    ...
    logger.error({ err: error }, 'Oracle analysis error');
    insights.push({
      type: 'risk',
      title: '数据分析异常',
...
export async function invokeAegisPower(): Promise<AegisStatus> {
  try {
    ...
    logger.error({ err: error }, 'Aegis status check error');
    return {
...
export async function recordConversation(userMessage: string, response: string) {
  ...
  try {
    ...
    logger.error({ err: error }, 'Failed to save conversation pattern');
  }
}
...
async function analyzeAndStorePatterns(): Promise<void> {
  ...
  if (significantTopics.length > 0) {
    try {
      ...
      logger.error({ err: error }, 'Failed to store pattern analysis');
    }
  }
}
...
export async function invokeGnosisPower(): Promise<GnosisMemory> {
  ...
  try {
    ...
    logger.error({ err: error }, 'Gnosis failed to fetch memories');
  }
...
export async function invokePrometheusPower(): Promise<PrometheusEvolution> {
  ...
  try {
    ...
    logger.error({ err: error }, 'Prometheus failed to fetch evolution data');
  }
