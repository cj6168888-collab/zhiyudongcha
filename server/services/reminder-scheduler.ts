...
class ReminderSchedulerService {
  ...
  setWebSocketBroadcaster(broadcaster: WebSocketBroadcaster): void {
    ...
    logger.info('[ReminderScheduler] WebSocket broadcaster configured');
  }
  ...
  async createRule(input: Omit<InsertReminderRule, 'id' | 'createdAt' | 'updatedAt'>): Promise<ReminderRule> {
    ...
    const [rule] = await getDatabase().insert(reminderRules).values({
      ...input,
      nextTriggerAt,
    }).returning();

    logger.info(`[ReminderScheduler] Rule created: ${rule.title} (${rule.id})`);
    return rule;
  }
  ...
  async getRules(options?: { 
    ...
    let query = getDatabase().select().from(reminderRules);
  ...
  async updateRule(id: string, updates: Partial<InsertReminderRule>): Promise<ReminderRule | null> {
    const [updated] = await getDatabase().update(reminderRules)
  ...
  async deleteRule(id: string): Promise<boolean> {
    const result = await getDatabase().delete(reminderRules)
      .where(eq(reminderRules.id, id));
  ...
  async processReminders(): Promise<ReminderPayload[]> {
    ...
    const dueRules = await getDatabase().select()
      .from(reminderRules)
      .where(and(
    ...
    logger.info(`[ReminderScheduler] Processing ${dueRules.length} due reminders`);
    ...
    for (const rule of dueRules) {
      try {
        ...
        logger.error({ err: error, ruleId: rule.id }, 'Failed to trigger rule');
      }
    }
  ...
  private async triggerReminder(rule: ReminderRule): Promise<ReminderPayload | null> {
    ...
    const [log] = await getDatabase().insert(reminderLogs).values({
      ruleId: rule.id,
      title: rule.title,
      content,
    ...
    await getDatabase().update(reminderRules)
      .set({
  ...
  private async generateReminderContent(rule: ReminderRule): Promise<string> {
    ...
    switch (rule.ruleType) {
      case 'CALENDAR_EVENT':
        if (rule.entityId) {
          const [event] = await getDatabase().select()
            .from(calendarEvents)
      ...
      case 'MILESTONE_DEADLINE':
        if (rule.entityId) {
          const [milestone] = await getDatabase().select()
      ...
      case 'TASK_DEADLINE':
        if (rule.entityId) {
          const [task] = await getDatabase().select()
            .from(projectTasks)
  ...
  private async deliverReminder(payload: ReminderPayload, channel: ReminderChannel, logId: string): Promise<void> {
    try {
      switch (channel) {
        ...
        case 'SMS':
          // SMS delivery requires external SMS gateway integration
          // Log as warning since this channel is configured but not available
          logger.warn({ 
            channel: 'SMS', 
            title: payload.title,
            reason: 'SMS网关未配置'
          }, '[ReminderScheduler] SMS投递需要配置外部短信网关服务');
          break;
        ...
        case 'EMAIL':
          ...
          logger.warn({ 
            ...
            title: payload.title,
            reason: 'SMTP服务未配置'
          }, '[ReminderScheduler] 邮件投递需要配置SMTP服务');
          break;
      }

      await getDatabase().update(reminderLogs)
        .set({
          status: 'DELIVERED',
      ...
      logger.info(`[ReminderScheduler] Delivered: ${payload.title} via ${channel}`);
    } catch (error) {
  ...
  private async checkFollowUpOpportunities(): Promise<void> {
    ...
    const contacts = await getDatabase().select()
      .from(persons)
      .where(or(
  ...
  private async checkProjectDeadlines(): Promise<void> {
    ...
    const upcomingMilestones = await getDatabase().select()
      .from(projectMilestones)
      .where(and(
  ...
  async getReminderLogs(options?: {
    ...
    let query = getDatabase().select().from(reminderLogs);
  ...
  async markReminderRead(logId: string): Promise<ReminderLog | null> {
    const [updated] = await getDatabase().update(reminderLogs)
  ...
  async getStats(): Promise<{
    ...
    const allRules = await getDatabase().select().from(reminderRules);
    const enabledRules = allRules.filter(r => r.enabled);

    const pendingLogs = await getDatabase().select()
      .from(reminderLogs)
      .where(eq(reminderLogs.status, 'PENDING'));

    const deliveredToday = await getDatabase().select()
      .from(reminderLogs)
      .where(and(
