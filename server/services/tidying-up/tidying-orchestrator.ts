...
const logger = createServiceLogger('TidyingOrchestrator');
...
export class TidyingOrchestrator {
  async createScanTask(deviceId: string, scanPath: string): Promise<string> {
    const [task] = await getDatabase().insert(tidyingTasks).values({
      deviceId,
      taskType: 'SCAN',
      status: 'PENDING',
      targetPath: scanPath,
  ...
  async processScan(taskId: string, scanRequest: ScanRequest): Promise<TidyingReport> {
    await getDatabase().update(tidyingTasks)
    ...
    try {
      ...
      await getDatabase().update(tidyingTasks).set({
        status: 'COMPLETED',
  ...
  async executePlan(taskId: string): Promise<TidyingTask> {
    const [task] = await getDatabase().select().from(tidyingTasks).where(eq(tidyingTasks.id, taskId));
  ...
  async updateActionStatus(
    ...
    if (actionIndex >= 0) {
      ...
      await getDatabase().update(tidyingTasks).set({ actions }).where(eq(tidyingTasks.id, taskId));
    }
  ...
  async getTaskHistory(deviceId: string, limit: number = 10): Promise<TidyingTask[]> {
    const tasks = await getDatabase().select()
      .from(tidyingTasks)
