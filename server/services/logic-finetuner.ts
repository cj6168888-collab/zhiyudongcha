...
const logger = createServiceLogger('LogicFinetuner');

import { getDatabase } from '../db';
import { ragKnowledge, avatarChatHistory, expertDecisions, shadowMemories, evolutionEvents, type AvatarChatHistory } from '@shared/schema';
...
class LogicFinetunerService {
  ...
  async analyzeConversations(days: number = 1): Promise<FineTuneResult> {
    logger.info(`[LogicFinetuner] 分析最近${days}天的对话...`);
    ...
    logger.info(`[LogicFinetuner] 发现${negotiationChats.length}条商业谈话`);
  ...
  private filterNegotiationChats(chats: AvatarChatHistory[]): AvatarChatHistory[] {
  ...
  private async extractNegotiationTactics(chats: AvatarChatHistory[]): Promise<NegotiationTactic[]> {
  ...
  private async identifyLogicTraps(chats: AvatarChatHistory[]): Promise<LogicTrap[]> {
  ...
  private calculateTrapMatchStrength(matches: AvatarChatHistory[], indicators: string[]): { confidence: number; probability: number } {
  ...
  async deepAnalyzeWithQwen(context: string): Promise<{
    ...
    logger.info('[LogicFinetuner] 进行深度分析...');
  ...
  async getLearnedTactics(category?: string): Promise<typeof ragKnowledge.$inferSelect[]> {
    const query = getDatabase().select().from(ragKnowledge);
