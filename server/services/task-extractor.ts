...
function parseJSONResponse(text: string): { tasks?: ParsedTaskData[]; summary?: string } | null {
...
export class TaskExtractorService {
  async extractFromEmail(email: Email): Promise<ExtractionResult> {
    ...
    try {
      ...
      const tasks: ExtractedTask[] = (parsed.tasks || []).map((t: ParsedTaskData) => ({
        source: 'email' as const,
        sourceId: email.id,
        title: t.title || '未命名任务',
        description: t.description || '',
        priority: (t.priority || 'normal') as ExtractedTask['priority'],
        category: (t.category || 'other') as ExtractedTask['category'],
        suggestedDeadline: parseDeadline(t.suggestedDeadline ?? null),
        deadlineSource: t.deadlineSource || null,
        assignee: t.assignee || null,
        relatedContacts: (t.relatedContacts || [email.fromEmail]).filter((c): c is string => c !== null),
      ...
      logger.error({ err: error }, 'Email extraction failed');
      return this.createEmptyResult('email', email.id);
    }
  }
  ...
  async batchExtractFromEmails(emails: Email[]): Promise<ExtractionResult[]> {
    ...
    for (const email of emails) {
      try {
        ...
        logger.error({ err: error, emailId: email.id }, 'Failed to extract from email');
      }
    }
