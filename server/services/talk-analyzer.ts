...
const logger = createServiceLogger('TalkAnalyzer');
...
export async function analyzeTalkContent(transcript: string): Promise<TalkAnalysisResult> {
  ...
  try {
    ...
    if (!response.ok) {
      logger.error({ status: response.status }, 'DashScope API error');
      return getDefaultAnalysisResult();
    }
    ...
    logger.error({ err: error }, 'Error analyzing talk content');
    return getDefaultAnalysisResult();
  }
}
...
- PITCH: 推销、路演、产品介绍` 
    ...
    logger.error({ err: error }, 'Error identifying talk type');
    return 'UNKNOWN';
  }
}
