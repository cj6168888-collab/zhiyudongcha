...
const logger = createServiceLogger('ActionOrchestrator');
...
class ActionOrchestratorService {
  ...
  async processSignal(signal: PerceptionSignal): Promise<OrchestrationResult> {
    logger.info(`[Orchestrator] 收到感知信号: ${signal.type} from ${signal.source}`);
  ...
  private evaluateCondition(condition: ScenarioCondition, data: Record<string, unknown>): boolean {
  ...
  private async executeScenario(
    ...
    logger.info(`[Orchestrator] 执行场景: ${scenario.name}`);
    ...
    for (const action of scenario.actions) {
      ...
      try {
        switch (action.type) {
          ...
          case 'NOTIFY':
            logger.info(`[Orchestrator] 通知 ${action.target}: ${action.params.message}`);
            break;
            
          case 'DISPLAY':
            logger.info(`[Orchestrator] 显示到 ${action.target}: ${action.params.message}`);
            break;
            
          case 'SCHEDULE':
            logger.info(`[Orchestrator] 日程操作: ${action.target}`);
            break;
            
          case 'DOCUMENT':
            logger.info(`[Orchestrator] 文档操作: ${action.target}`);
            break;
        }
      } catch (error) {
        logger.error({ error }, '执行动作失败');
      }
    }
  }
  ...
  addScenario(scenario: ActionScenario): void {
    ...
    logger.info(`[Orchestrator] 场景已添加/更新: ${scenario.name}`);
  }
  ...
  async simulateScenario(scenarioId: string, testData: Record<string, unknown>): Promise<OrchestrationResult | null> {
