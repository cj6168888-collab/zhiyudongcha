...
const logger = createServiceLogger('FileIndexer');
...
let pdfParse: ((buffer: Buffer) => Promise<{ text: string; numpages: number; info: Record<string, unknown> }>) | null = null;
...
export class FileIndexer {
  ...
  private async parsePDF(buffer: Buffer, fileName: string): Promise<ParsedContent> {
    try {
      ...
      logger.error({ error, fileName }, 'PDF解析失败');
  ...
  private async parseWord(buffer: Buffer, fileName: string): Promise<ParsedContent> {
    try {
      ...
      logger.error({ error, fileName }, 'Word解析失败');
  ...
  private async parseArchive(buffer: Buffer, fileName: string): Promise<ParsedContent> {
    try {
      ...
      logger.error({ error, fileName }, '压缩包解析失败');
  ...
  async indexFile(file: FileUpload): Promise<FileKnowledge> {
    ...
    const [created] = await getDatabase().insert(fileKnowledge).values({
  ...
  private async processFileAsync(fileId: string, file: FileUpload): Promise<void> {
    try {
      await getDatabase().update(fileKnowledge)
      ...
      await getDatabase().update(fileKnowledge).set({
        extractedText: text,
...
请以JSON格式输出：
{
  ...
  private async findByHash(hash: string): Promise<FileKnowledge | null> {
    if (!hash) return null;
    const [existing] = await getDatabase().select().from(fileKnowledge).where(eq(fileKnowledge.fileHash, hash));
    return existing || null;
  }

  async getFile(id: string): Promise<FileKnowledge | null> {
    const [file] = await getDatabase().select().from(fileKnowledge).where(eq(fileKnowledge.id, id));
    return file || null;
  }

  async searchFiles(query: string): Promise<FileKnowledge[]> {
    return getDatabase().select().from(fileKnowledge)
      .where(or(
