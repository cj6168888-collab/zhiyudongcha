...
const logger = createServiceLogger('SocialGraph');
...
export class SocialGraph {
  ...
  async createNode(data: InsertSocialGraphNode): Promise<SocialGraphNode> {
    const [node] = await getDatabase().insert(socialGraphNodes).values(data).returning();
    return node;
  }

  async createEdge(data: InsertSocialGraphEdge): Promise<SocialGraphEdge> {
    const [edge] = await getDatabase().insert(socialGraphEdges).values(data).returning();
    return edge;
  }

  async getNodeByContactId(contactId: string): Promise<SocialGraphNode | null> {
    const [node] = await getDatabase().select().from(socialGraphNodes)
  ...
  async getEdge(sourceNodeId: string, targetNodeId: string): Promise<SocialGraphEdge | null> {
    const [edge] = await getDatabase().select().from(socialGraphEdges)
      .where(and(
  ...
  async getFullGraph(): Promise<GraphData> {
    const nodes = await getDatabase().select().from(socialGraphNodes);
    const edges = await getDatabase().select().from(socialGraphEdges);
    return { nodes, edges };
  }

  async getHighInfluenceNodes(limit: number = 10): Promise<SocialGraphNode[]> {
    return getDatabase().select().from(socialGraphNodes)
  ...
  async updateNodePosition(nodeId: string, posX: number, posY: number): Promise<void> {
    await getDatabase().update(socialGraphNodes).set({
      posX,
      posY,
      updatedAt: new Date()
  ...
  async recordInteraction(sourceContactId: string, targetContactId: string): Promise<void> {
    ...
    if (sourceNode && targetNode) {
      ...
      if (edge) {
        await getDatabase().update(socialGraphEdges).set({
