...
const logger = createServiceLogger('InferenceQueue');
...
export class InferenceQueue extends EventEmitter {
  ...
  addJob(
    ...
    logger.info(`[InferenceQueue] Job added: ${job.id} (${type}, ${job.priority})`);
  ...
  cancelJob(jobId: string): boolean {
    ...
    logger.info(`[InferenceQueue] Job cancelled: ${jobId}`);
    return true;
  }
  ...
  private async executeJob(job: InferenceJob): Promise<void> {
    if (!this.jobProcessor) {
      logger.error('[InferenceQueue] No job processor set');
      job.status = 'FAILED';
    ...
    logger.info(`[InferenceQueue] Job started: ${job.id}`);
    ...
    try {
      ...
      logger.info(`[InferenceQueue] Job completed: ${job.id} in ${job.completedAt - (job.startedAt || job.createdAt)}ms`);
      ...
      if (errorMessage === 'Job timeout') {
        job.status = 'TIMEOUT';
        logger.info(`[InferenceQueue] Job timeout: ${job.id}`);
      } else if (job.retryCount < job.config.maxRetries) {
        // 重试
        job.retryCount++;
        job.status = 'PENDING';
        logger.info(`[InferenceQueue] Job retry ${job.retryCount}/${job.config.maxRetries}: ${job.id}`);
        this.emit('job:retry', job);
      } else {
        job.status = 'FAILED';
        job.error = errorMessage;
        logger.info(`[InferenceQueue] Job failed: ${job.id} - ${errorMessage}`);
  ...
  cleanup(maxAge: number = 3600000): number {
    ...
    if (cleaned > 0) {
      logger.info(`[InferenceQueue] Cleaned ${cleaned} old jobs`);
    }
