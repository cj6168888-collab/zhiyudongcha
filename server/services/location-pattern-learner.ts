...
const logger = createServiceLogger('LocationPatternLearner');
...
class LocationPatternLearnerService {
  ...
  async recordVisit(data: {
    ...
    const [visit] = await getDatabase().insert(locationVisits).values({
    ...
    logger.info(`[LocationLearner] 访问已记录: ${data.locationName}`);
  ...
  private async findOrCreatePattern(data: {
    ...
    if (data.latitude && data.longitude) {
      const nearby = await getDatabase().select().from(locationPatterns)
        .where(
          and(
    ...
    if (!pattern) {
      const byName = await getDatabase().select().from(locationPatterns)
    ...
    if (!pattern) {
      const [newPattern] = await getDatabase().insert(locationPatterns).values({
      ...
      pattern = newPattern;
      logger.info(`[LocationLearner] 新地点已识别: ${data.locationName}`);
    }
  ...
  private async updatePatternStats(patternId: string, visit: LocationVisit): Promise<LocationPattern> {
    const allVisits = await getDatabase().select().from(locationVisits)
    ...
    const [updated] = await getDatabase().update(locationPatterns)
      .set({
        visitCount,
  ...
  async predictNextLocation(currentTime: Date = new Date()): Promise<LocationPrediction[]> {
    ...
    const patterns = await getDatabase().select().from(locationPatterns)
  ...
  async getFrequentLocations(): Promise<LocationPattern[]> {
    return await getDatabase().select().from(locationPatterns)
  ...
  async getDailyLocationSummary(date: Date = new Date()): Promise<DailyLocationSummary> {
    ...
    const visits = await getDatabase().select({
  ...
  async updateVisitDeparture(visitId: string, departureTime: Date, postVisitEnergy?: number): Promise<LocationVisit | null> {
    const [visit] = await getDatabase().select().from(locationVisits)
    ...
    const [updated] = await getDatabase().update(locationVisits)
      .set({
        departureTime,
        durationMinutes,
        postVisitEnergy,
