...
const logger = createServiceLogger('PsychProfiler');
...
class PsychProfilerService {
  ...
  constructor() {
    ...
    logger.info('[PsychProfiler] 心理侧写服务已初始化');
  }
...
- 保持客观专业，避免过于绝对的判断`
      ...
      if (!response.ok) {
        ...
        logger.error({ errorText }, 'API error');
  ...
  async createOrUpdateProfile(
    ...
    if (existing.length) {
      // 更新现有侧写
      const [updated] = await getDatabase().update(psychProfiles)
        .set({
          ...analysis.profile,
          sourceType,
      ...
      // 创建新侧写
      const [created] = await getDatabase().insert(psychProfiles)
        .values({
          personId,
          ...analysis.profile,
          sourceType,
  ...
  async addObservationNote(
    ...
    await getDatabase().update(psychProfiles)
      .set({
