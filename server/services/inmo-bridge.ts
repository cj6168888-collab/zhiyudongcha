...
const logger = createServiceLogger('InmoBridge');
...
class INMOBridgeService {
  ...
  async translateText(
    ...
    if (cached) {
      logger.info(`[INMO Bridge] 翻译缓存命中: ${text.substring(0, 30)}...`);
      return cached;
    }

    if (!DASHSCOPE_API_KEY) {
      logger.warn('[INMO Bridge] DashScope API未配置');
      return {
        sourceText: text,
        sourceLanguage,
...
4. 如果原文已经是目标语言，直接返回原文`;
      ...
      logger.info(`[INMO Bridge] 翻译完成: "${text.substring(0, 30)}..." -> "${translatedText.substring(0, 30)}..." (${result.processingTimeMs}ms)`);

      return result;
    } catch (error) {
      logger.error({ err: error }, '[INMO Bridge] 翻译失败');
      return {
        sourceText: text,
        sourceLanguage,
  ...
  async performOCR(request: OCRRequest): Promise<OCRResult> {
    if (!DASHSCOPE_API_KEY) {
      logger.warn('[INMO Bridge] DashScope API未配置，无法执行OCR');
      return {
        text: '',
        confidence: 0,
        language: 'unknown',
    ...
    try {
      ...
      logger.info(`[INMO Bridge] OCR完成: "${recognizedText.substring(0, 50)}..." (${detectedLanguage})`);
      ...
      logger.error({ err: error }, '[INMO Bridge] OCR失败');
      return {
        text: '',
        confidence: 0,
        language: 'unknown',
  ...
  async performOCRAndTranslate(
    ...
    if (ocrResult.language === targetLanguage) {
      logger.info('[INMO Bridge] 识别语言与目标语言相同，跳过翻译');
      return { ocr: ocrResult, translation: null };
    }
  ...
  parseVoiceCommand(transcript: string): VoiceCommand | null {
    ...
    for (const pattern of patterns) {
      if (pattern.regex.test(normalizedText)) {
        logger.info(`[INMO Bridge] 识别语音指令: ${pattern.type}`);
  ...
  registerClient(ws: WebSocket, deviceId: string, deviceName: string): INMOBridgeClient {
    ...
    logger.info(`[INMO Bridge] 客户端已注册: ${deviceName} (${clientId})`);
  ...
  unregisterClient(clientId: string): boolean {
    ...
    if (client) {
      ...
      logger.info(`[INMO Bridge] 客户端已断开: ${client.deviceName} (${clientId})`);
  ...
  startTranslation(clientId: string, config: TranslationConfig): boolean {
    ...
    if (client) {
      ...
      logger.info(`[INMO Bridge] 开始翻译: ${client.deviceName} (${config.sourceLanguage} -> ${config.targetLanguage})`);
  ...
  stopTranslation(clientId: string): boolean {
    ...
    if (client) {
      ...
      logger.info(`[INMO Bridge] 停止翻译: ${client.deviceName}`);
  ...
  async sendToClient(clientId: string, message: Record<string, unknown>): Promise<boolean> {
    const client = this.clients.get(clientId);
  ...
  async broadcastToAll(message: Record<string, unknown>): Promise<number> {
    let sent = 0;
  ...
  clearCache(): void {
    ...
    logger.info('[INMO Bridge] 翻译缓存已清空');
  }
}
