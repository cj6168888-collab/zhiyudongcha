...
async function getReadState(): Promise<Record<string, boolean>> {
  try {
    return await cryptoStorage.getItem<Record<string, boolean>>('dream_insight_read_state', {});
  } catch {
    return {};
  }
}

async function saveReadState(state: Record<string, boolean>): Promise<void> {
  try {
    await cryptoStorage.setItem('dream_insight_read_state', state);
  } catch {
  }
}
...
export const useDreamInsightStore = create<DreamInsightStore>((set, get) => ({
  ...
  dismissBubble: async () => {
    ...
    if (currentInsight) {
      await markAsRead(currentInsight.id);
    }
