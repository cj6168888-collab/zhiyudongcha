...
export function AvatarChat() {
  ...
  const [cloudVoiceMode, setCloudVoiceMode] = useState(false);
  ...
  const pttRecognitionRef = useRef<SpeechRecognition | null>(null);
  ...
  const handleCloudAssistantResponse = useCallback((text: string) => {
    if (text.trim()) {
      addMessage({
        role: 'assistant',
        content: text,
  ...
  const handleCloudTranscript = useCallback((text: string, isFinal: boolean) => {
  ...
  const {
    ...
    isListening: cloudListening,
    isSpeaking: cloudSpeaking,
    interimTranscript: cloudInterimTranscript,
    error: cloudError,
    connect: cloudConnect,
    disconnect: cloudDisconnect,
    startListening: cloudStartListening,
    stopListening: cloudStopListening,
    interrupt: cloudInterrupt,
    sendTextMessage: cloudSendText,
  } = useRealtimeVoice(handleCloudAssistantResponse, handleCloudTranscript, {
    userId: 'default',
    autoConnect: false,
  });
  ...
  const handleUserInput = async (text: string) => {
    ...
    try {
      ...
      if (result.data?.refreshKey) {
        queryClient.invalidateQueries({ queryKey: [result.data.refreshKey] });
      }
  ...
  const startPushToTalk = useCallback(() => {
    if (pushToTalkActive || naturalMode || cloudVoiceMode) return;

    const SpeechRecognition = window.SpeechRecognition || (window as unknown as { webkitSpeechRecognition: typeof window.SpeechRecognition }).webkitSpeechRecognition;
    if (!SpeechRecognition) {
      console.error('[AvatarChat] SpeechRecognition not supported');
      return;
    }
    ...
    recognition.onerror = (event: SpeechRecognitionErrorEvent) => {
      console.error('[AvatarChat] PTT error:', event.error);
    ...
    recognition.start();
  }, [pushToTalkActive, naturalMode, cloudVoiceMode]);

  const stopPushToTalk = useCallback(() => {
    if (!pushToTalkActive || !pttRecognitionRef.current) return;
    ...
    if (pttTranscript.trim()) {
      handleUserInput(pttTranscript.trim());
    }
    setPttTranscript("");
  }, [pushToTalkActive, pttTranscript, handleUserInput]);
  ...
  const toggleCloudVoiceMode = async () => {
    if (cloudVoiceMode) {
      cloudStopListening();
      cloudDisconnect();
      setCloudVoiceMode(false);
    } else {
      setCloudVoiceMode(true);
      await cloudConnect();
    }
  };

  const handleCloudVoiceToggle = async () => {
    if (cloudConnected) {
      if (cloudListening) {
        cloudStopListening();
      } else {
  ...
  const getStateLabel = () => {
    if (cloudVoiceMode) {
      if (!cloudConnected) return '连接中...';
      if (cloudSpeaking) return '回复中...';
      if (cloudListening) return '云端聆听...';
      switch (cloudState) {
        case 'processing': return '思考中...';
        case 'speaking': return '播放中...';
        case 'listening': return '云端聆听...';
        case 'idle': return '云端就绪';
        default: return '云端语音';
      }
    }
  ...
  if (chatMinimized) {
    return (
      ...
      >
        ...
        <span className="text-sm">小星</span>
    ...
    >
      <div className="flex items-center justify-between p-3 border-b border-border bg-secondary/30">
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-2">
            <span className="font-medium text-sm">小星</span>
      ...
      {showSettings && (
        <div className="p-3 border-b border-border bg-secondary/20 space-y-3" data-testid="chat-settings-panel">
          ...
          <div className="flex items-center justify-between text-sm">
            <div className="flex items-center gap-2">
              <Wifi className={cn("w-4 h-4", cloudVoiceMode && cloudConnected ? "text-blue-500" : "text-muted-foreground")} />
              <div>
                <span>云端语音 (DashScope)</span>
                {cloudVoiceMode && cloudConnected && <Badge variant="default" className="ml-2 text-[9px] bg-blue-500">已连接</Badge>}
              </div>
            </div>
            <Button
              variant={cloudVoiceMode ? "default" : "outline"}
              size="sm"
              className="h-6 text-xs"
              onClick={toggleCloudVoiceMode}
              data-testid="button-cloud-voice-toggle"
            >
              {cloudVoiceMode ? "关闭" : "开启"}
            </Button>
          </div>

          {cloudVoiceMode && (
            <div className="text-[10px] text-muted-foreground bg-blue-500/10 rounded p-2">
              云端语音：使用DashScope实时语音服务，识别更准确，支持高质量TTS。
              {cloudListening && <span className="text-blue-500 ml-1">正在聆听...</span>}
            </div>
          )}
      ...
      {cloudVoiceMode && cloudInterimTranscript && (
        <div className="px-3 py-2 bg-blue-500/10 border-b border-border">
          <div className="flex items-center gap-2">
            <Wifi className="w-3 h-3 text-blue-500 animate-pulse" />
            <span className="text-xs text-muted-foreground italic">{cloudInterimTranscript}</span>
          </div>
        </div>
      )}

      {!cloudVoiceMode && naturalMode && interimTranscript && (
      ...
      <ScrollArea className="flex-1 p-3 min-h-[300px]">
        {messages.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center py-8">
            ...
            <p className="text-sm text-muted-foreground mb-2">你好，我是小星</p>
      ...
      <div className="p-3 border-t border-border bg-secondary/20">
        <div className="flex gap-2">
          {cloudVoiceMode ? (
            <Button
              variant={cloudListening ? "default" : "outline"}
              size="icon"
              className={cn("h-9 w-9 shrink-0", cloudListening && "bg-blue-600 hover:bg-blue-700")}
              onClick={handleCloudVoiceToggle}
              disabled={!cloudConnected}
              data-testid="button-cloud-voice"
            >
              {cloudListening ? <MicOff className="w-4 h-4" /> : <Mic className="w-4 h-4" />}
            </Button>
          ) : naturalMode ? (
            <Button
            ...
            <Button
              ...
              size="icon"
              className={cn("h-9 w-9 shrink-0", pushToTalkActive && "bg-red-500 hover:bg-red-600 animate-pulse")}
            ...
            >
              {pushToTalkActive ? <MicOff className="w-4 h-4" /> : <Mic className="w-4 h-4" />}
            </Button>
          )}
          <Input
            placeholder={pushToTalkActive ? pttTranscript || "正在录音..." : cloudVoiceMode ? "云端语音中，点击麦克风开始..." : naturalMode ? "自然对话中，直接说话即可..." : "按住麦克风说话或输入文字..."}
            value={inputText}
            onChange={(e) => setInputText(e.target.value)}
            onKeyDown={(e) => e.key === 'Enter' && handleSend()}
            className="h-9 text-sm"
            disabled={isProcessing || cloudState === 'processing'}
          ...
          <Button
            ...
            onClick={handleSend}
            disabled={!inputText.trim() || isProcessing || cloudState === 'processing'}
        ...
        {cloudVoiceMode && (
          <div className="mt-2 flex items-center justify-between text-[10px] text-muted-foreground">
            <span className="flex items-center gap-1">
              <Wifi className={cn("w-3 h-3", cloudConnected ? "text-blue-500" : "text-muted-foreground")} />
              {cloudConnected ? "已连接云端" : "连接中..."}
              {cloudSpeaking && <span className="text-blue-500 ml-1">播放中...</span>}
            </span>
            <Button
              variant="ghost"
              size="sm"
              ...
              data-testid="button-exit-cloud-mode"
            >
              退出云端语音
            </Button>
          </div>
        )}
        {!cloudVoiceMode && naturalMode && (
          <div className="mt-2 flex items-center justify-between text-[10px] text-muted-foreground">
            <span>说 "小星" 唤醒</span>
            <Button
              variant="ghost"
