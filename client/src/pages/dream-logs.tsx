    ...
    if (evolutionEvents.length > 0) {
      evolutionEvents.slice(0, 5).forEach((event: any, index: number) => {
        insights.push({
          ...
          description: event.deltaDescription || '小星有了新的成长',
  ...
  const pushInsightToChat = (insight: EvolutionInsight) => {
    toast.success(
      <div className="flex flex-col gap-1">
        <span className="font-medium">小星学习报告</span>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <CardDescription>小星的学习报告与成长发现</CardDescription>
                      </div>
                    </div>
                    <Button
                      variant="outline"
                ...
                <CardContent>
                  {insights.length === 0 ? (
                    <div className="text-center py-8 text-slate-500">
                      ...
                      <p className="text-xs mt-1">小星正在努力学习中...</p>
                    </div>
                  ) : (
        ...
        {isLoading ? (
          ...
          <Card className="border-slate-700 bg-gradient-to-br from-slate-800 to-purple-900/20">
            <CardContent className="py-16 text-center">
              ...
              <h3 className="text-xl font-medium text-slate-300 mb-2">小星还没有做过梦</h3>
              <p className="text-sm text-slate-500 max-w-md mx-auto">
                深夜时分，当系统进入闲置状态，小星会自动进入梦境模式，
                进行商业推演、知识蒸馏与系统自举进化。
                <br /><br />
