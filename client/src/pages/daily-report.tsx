...
import type { InsightsProcessing } from "@/shared/types";
...
export default function DailyReportPage() {
  ...
  const generateMutation = useMutation({
    mutationFn: async () => {
      ...
      const battleReport = await apiRequest('POST', '/api/report/generate', { date: new Date().toISOString() }) as { report?: { summary?: string } };

      const payload = {
        reportDate: new Date().toISOString(),
        decisionSummary: battleReport.report?.summary || '小星正在为您整理今日信息...',
        opportunitiesFound: [],
        predictions: [],
        riskAlerts: [],
        tasksCompleted: 0,
    ...
    onSuccess: () => {
      ...
      toast({ title: "日报已生成", description: "小星已为您整理今日汇报", className: "border-primary" });
    },
    onError: () => {
      setIsGenerating(false);
  ...
  return (
    <div className="min-h-screen bg-background px-6 pt-6 pb-24 md:pb-6" data-testid="daily-report-page">
      <div className="max-w-7xl mx-auto space-y-6">
        <GlobalWakeHeader
          title="每日汇报板"
          subtitle="小星为您整理的今日情报"
          rightActions={
            <div className="flex items-center gap-3">
              <Badge variant="outline" className="bg-primary/20">
                HP: {hpBalance.toLocaleString()}
              </Badge>
              {!todayReport && role === 'MASTER' && (
                <Button
          ...
          <Card className="border-primary/50 bg-gradient-to-br from-primary/5 to-transparent">
            <CardContent className="flex items-center justify-center py-12">
              <div className="text-center space-y-4">
                ...
                <div>
                  <p className="text-lg font-medium">小星正在整理今日信息...</p>
                  <p className="text-sm text-muted-foreground">分析会谈记录、项目进展、人脉动态</p>
                </div>
              </div>
            </CardContent>
          </Card>
        ...
        <Card className="border-purple-500/30">
          <CardHeader>
            ...
            <CardDescription>小星生成的文书、合同、报告等文件</CardDescription>
          </CardHeader>
          <CardContent>
            {generatedFiles.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-32 text-muted-foreground">
                ...
                <p>暂无生成文件</p>
                <p className="text-xs">通过对话让小星生成合同、策划书等文件</p>
              </div>
            ) : (
